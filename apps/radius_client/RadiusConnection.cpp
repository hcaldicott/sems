...
RadiusConnection::RadiusConnection(unsigned int connection_id, string &name, string &server, unsigned short port,
                                   string &secret, unsigned int timeout_msec, unsigned int attempts)
...
int RadiusConnection::init()
{
    ...
    dns_handle dh;
    if (-1 == resolver::instance()->resolve_name(server, &dh, &a, IPv4_only)) {
...
void RadiusConnection::process()
{
    ...
    DBG("got reply with code %d for session %s", reply.code(), request->session().c_str());
...
int RadiusConnection::avp_info::parse(const AmArg &a)
{
    try {
        ...
        if (fmt_name == "string")
            fmt = str;
        else if (fmt_name == "integer")
            fmt = integer;
        else if (fmt_name == "octets")
            fmt = octets;
        else if (fmt_name == "date")
            fmt = date;
        else if (fmt_name == "ipaddr")
            fmt = ipv4;
        else if (fmt_name == "ip6addr")
            fmt = ipv6;
        else {
...
#define PLACEHOLDER_WRAPPER '$'
    ...
    while ((pos = value.find(PLACEHOLDER_WRAPPER, pos)) != string::npos) {
        if (!arg) {
            ...
            if (start == pos) { // empty placeholder (escaping)
                ...
                map<string, string>::const_iterator it = values_hash.find(placeholder_name);
                if (it != values_hash.end()) {
                    ss << it->second.c_str();
                } else {
                    WARN("uknown variable name '%s' in placeholder. leave it as is", placeholder_name.c_str());
                    ss << PLACEHOLDER_WRAPPER << placeholder_name << PLACEHOLDER_WRAPPER;
    ...
    if (arg) {
        ERROR("unbalanced placeholder wrappers count in %s."
              "use %c%c to place %c",
              value.c_str(), PLACEHOLDER_WRAPPER, PLACEHOLDER_WRAPPER, PLACEHOLDER_WRAPPER);
        return 1;
    }
    ...
    DBG("add avp: %d:%s:%s with value %s", type, name.c_str(), fmt_name.c_str(), v.c_str());

    switch (fmt) {
    case str:
    case octets:
        if (vsa)
            return p->add_vendor_attr(type, vsa_vendor_id, vsa_vendor_type, v.data(), v.size());
        else
            return p->add_attr_string(type, v);
        break;
    case integer:
    case date:
        if (!str2int(v, i)) {
        ...
        if (vsa)
            return p->add_vendor_attr_int32(type, vsa_vendor_id, vsa_vendor_type, i);
        else
            return p->add_attr_int32(type, i);
        break;
    case ipv4:
        ...
        if (vsa)
            return p->add_vendor_attr(type, vsa_vendor_id, vsa_vendor_type, (const char *)&(SAv4(&a)->sin_addr),
                                      sizeof(struct in_addr));
        else
            return p->add_attr(type, (const char *)&(SAv4(&a)->sin_addr), sizeof(struct in_addr));
        break;
    case ipv6:
        if (1 != am_inet_pton(v.c_str(), &a)) {
            ERROR("can't convert %s to IP address", v.c_str());
        ...
        if (vsa)
            return p->add_vendor_attr(type, vsa_vendor_id, vsa_vendor_type, (const char *)&(SAv6(&a)->sin6_addr),
                                      sizeof(struct in6_addr));
        else
            return p->add_attr(type, (const char *)&(SAv6(&a)->sin6_addr), sizeof(struct in6_addr));
        break;
    }
...
void RadiusConnection::check_timeouts()
{
    ...
    for (SentMap::iterator it = sent_map.begin(); it != sent_map.end(); it++) {
        p = it->second;
