...
template <class T, class DT, int scale> string signed2str(T val, T (*abs_func)(T), DT (*div_func)(T, T))
{
    char buffer[64] = { 0, 0 };
    int  i          = 62;
    DT   d;
...
{
    return signed2str<int, div_t, 16>(val, abs, div);
}
string int2str(int val)
{
    return signed2str<int, div_t, 10>(val, abs, div);
}
string long2str(long int val)
{
    return signed2str<long, ldiv_t, 10>(val, labs, ldiv);
}
string longlong2str(long long int val)
{
    return signed2str<long long, lldiv_t, 10>(val, llabs, lldiv);
}
...
bool reverse_hex2int(const string &str, unsigned int &result)
{
    ...
    for (string::const_reverse_iterator pc = str.rbegin(); pc != str.rend(); ++pc) {
...
std::optional<std::pair<string, uint16_t>> parse_hostport(const std::string &host_port, bool allow_fqdn)
{
    ...
    for (auto const &c : host_port) {
        ...
        case host_part_start:
            ...
            case 'A' ... 'Z':
                if (!allow_fqdn) {
                    ERROR("fqdn is not allowed for host_port string '%s'", host_port.data());
                    return std::nullopt;
                }
                st = host_part_fqdn;
                break;
            default:
                ERROR("unexpected starting char '%c' in host_port string '%s'", c, host_port.data());
                return std::nullopt;
            }
            break;
        case host_part_fqdn:
            ...
            case ':':
                ...
                break;
            default:
                ERROR("unexpected fqdn char '%c' in host_port string '%s'", c, host_port.data());
                return std::nullopt;
            }
            break;
        case host_part_ipv4:
            ...
            case ':':
                ...
                break;
            default:
                ERROR("unexpected ipv4 address char '%c' in host_port string '%s'", c, host_port.data());
                return std::nullopt;
            }
            break;
        case host_part_ipv6_reference:
            ...
            case ']':
                ...
                break;
            default:
                ERROR("unexpected ipv6 address char '%c' in host_port string '%s'", c, host_port.data());
                return std::nullopt;
            }
            break;
        case host_port_delimiter:
            if (c != ':') {
                ERROR("unexpected char '%c' after the ipv6 reference in host_port string '%s'", c, host_port.data());
                return std::nullopt;
            }
            port_start_pos = pos + 1;
            st             = port_part;
        ...
        case port_part:
            switch (c) {
            case '0' ... '9': port = port * 10 + c - '0'; break;
            default:
                ERROR("unexpected char '%c' for port in host_port string '%s'", c, host_port.data());
                return std::nullopt;
            }
            break;
        }
    ...
    if (ipv4 && 1 != validate_ipv4_addr(host)) {
        ERROR("incorrect IPv4 address '%s' for host_port string '%s'", host.data(), host_port.data());
        return std::nullopt;
    }

    if (ipv6 && 1 != validate_ipv6_addr(host)) {
        ERROR("incorrect IPv6 address '%s' for host_port string '%s'", host.data(), host_port.data());
        return std::nullopt;
    }
...
{
    sockaddr_in6 addr;
    if (s.size() > 2 && s[0] != '[' && 1 == inet_pton(AF_INET6, s.c_str(), &addr)) {
        s.insert(0, "[");
        s.append("]");
    }
}
...
bool key_in_list(const string &s_list, const string &key, char list_delim)
{
    ...
    while (pos < s_list.length()) {
        ...
        while ((pos2 > 0) && ((s_list[pos2] == ' ') || (s_list[pos2] == list_delim) || (s_list[pos2] == '\n')))
            pos2--;
        ...
        while ((pos_n < s_list.length()) &&
               ((s_list[pos_n] == ' ') || (s_list[pos_n] == list_delim) || (s_list[pos_n] == '\n')))
            pos_n++;
...
string strip_header_params(const string &hdr_string)
{
    ...
    for (; (val_begin < hdr_string.length()) && hdr_string[val_begin] == ' '; val_begin++)
        ;
    // strip parameters
...
string get_header_param(const string &hdr_string, const string &param_name)
{
    ...
    while (pos < hdr_string.length()) {
        ...
        if ((hdr_string.length() > pos + param_name.length() + 1) &&
            hdr_string.substr(++pos, param_name.length()) == param_name && hdr_string[pos + param_name.length()] == '=')
        {
...
std::vector<string> explode(const string &s, const string &delim, const bool keep_empty)
{
...
bool read_regex_mapping(const string &fname, const char *sep, const char *dbg_type, RegexMappingVector &result)
{
    std::ifstream appcfg(fname.c_str());
    if (!appcfg.good()) {
        ERROR("could not load %s file at '%s'", dbg_type, fname.c_str());
        return false;
    } else {
        while (!appcfg.eof()) {
            string entry;
            ...
            if (re_v.size() != 2) {
                ERROR("Incorrect line '%s' in %s: expected format 'regexp%sstring'", entry.c_str(), fname.c_str(), sep);
                return false;
            }
            regex_t app_re;
            if (regcomp(&app_re, re_v[0].c_str(), REG_EXTENDED)) {
                ERROR("compiling regex '%s' in %s.", re_v[0].c_str(), fname.c_str());
                return false;
            }
            DBG("adding %s '%s' => '%s'", dbg_type, re_v[0].c_str(), re_v[1].c_str());
...
bool run_regex_mapping(const RegexMappingVector &mapping, const char *test_s, string &result)
{
    ...
    for (RegexMappingVector::const_iterator it = mapping.begin(); it != mapping.end(); it++) {
        if (!regexec(&it->first, test_s, MAX_GROUPS, groups, 0)) {
            ...
            for (g = 1; g < MAX_GROUPS; g++) {
                if (groups[g].rm_so == (int)(size_t)-1)
                    break;
                DBG("group %u: [%2u-%2u]: %.*s", g, groups[g].rm_so, groups[g].rm_eo, groups[g].rm_eo - groups[g].rm_so,
                    test_s + groups[g].rm_so);
                std::string match(test_s + groups[g].rm_so, groups[g].rm_eo - groups[g].rm_so);
...
void inplaceHeaderErase(string &hdrs, const char *hdr)
{
    ...
    while (start_pos < hdrs.length()) {
        ...
        if (0 != skip_header(hdrs, start_pos, name_end, val_begin, val_end, hdr_end))
            return;
