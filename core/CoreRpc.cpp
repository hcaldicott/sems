...
{
    INFO("change system dump_level from %s to %s", dump_level2str(AmConfig.dump_level), dump_level2str(dump_level));
...
static const int dump_level_names_count = (sizeof(dump_level_names) / sizeof(dump_level_names[0])) - 1;
...
void CoreRpc::set_system_shutdown(bool shutdown)
{
    ...
    if (AmConfig.shutdown_mode) {
        if (AmSession::getTerminateOnNoSessions() && !AmSession::getSessionNum()) {
...
void CoreRpc::init_rpc_tree()
{
    ...
    reg_method(show, "payloads", "", "", &CoreRpc::showPayloads, this);
...
int CoreRpc::onLoad()
{
    ...
    if (!AmConfig.log_dump_path.empty() && check_dir_write_permissions(AmConfig.log_dump_path)) {
        return -1;
    }
...
void CoreRpc::showPayloads(const AmArg &args, AmArg &ret)
{
    ...
    bool compute_cost = args.size() && isArgCStr(args[0]) && args[0] == "benchmark";
    ...
    for (; it != payloads.end(); ++it) {
        ...
        DBG("process codec: %s (%d)", p.encoding_name.c_str(), p.payload_type);
...
void CoreRpc::restartSslKeyLog(const AmArg &, AmArg &ret)
{
    ...
    if (ssl_key_log_filepath.empty()) {
        ...
        ssl_key_log_filepath = format("{}/secrets_{}.skl", AmConfig.log_dump_path, t.tv_sec);
    }
...
void CoreRpc::showStatus(const AmArg &, AmArg &ret)
{
    ...
    ret["shutdown_request_time"] = !timerisset(&last_shutdown_time) ? AmArg() : timeval2str(last_shutdown_time);
...

{
    for (MEDIA_interface &interface : AmConfig.media_ifs) {
        ...
        for (MEDIA_info *protoif : interface.proto_info) {
            ...
            protoif->iterateUsedPorts(
                [&proto_used_ports](const std::string &address, unsigned short rtp, unsigned short rtcp) {
...
void CoreRpc::showSessionsInfo(const AmArg &args, AmArg &ret)
{
    ...
    if (!args.size()) {
        AmEventDispatcher::instance()->iterate([&ret](const string &key, const AmEventDispatcher::QueueEntry &entry) {
        ...
        AmEventDispatcher::instance()->apply(local_tag, [&session_info](const AmEventDispatcher::QueueEntry &entry) {
        ...
        if (isArgStruct(session_info) && session_info.hasMember("other_id")) {
            const string other_local_tag    = session_info["other_id"].asCStr();
            AmArg       &other_session_info = ret[other_local_tag];

            AmEventDispatcher::instance()->apply(other_local_tag,
...
void CoreRpc::setShutdownAutoTerm(const AmArg &args, AmArg &ret)
{
    ...
    INFO("set auto termination on graceful shutdown: %d -> %d", AmSession::getTerminateOnNoSessions(), flag);
