...
int AmRtpAudio::receive(unsigned long long system_ts)
{
    ...
    while (true) {
        size =
            AmRtpStream::receive(static_cast<unsigned char *>(samples), static_cast<unsigned int>(AUDIO_BUFFER_SIZE));
        ...
        if (decoded_size <= 0) {
            if (rtp_stats.current_rx) {
                if (!rtp_stats.current_rx->decode_err) { // print just first decode error for current stream
                    DBG("AmAudio:decode(%d) returned %i. local_ssrc: 0x%x, local_tag: %s", size, decoded_size, l_ssrc,
                        session ? session->getLocalTag().c_str() : "no session");
                }
        ...
        playout_buffer->write(wallclock_ts, static_cast<u_int32_t>(adjusted_rtp_ts),
        ...
        if (!active && !last_recv_relayed) {
            DBG("switching to active-mode\t(ts=%llu;stream=%p)", last_recv_ts, static_cast<void *>(this));
            active = true;
        }
    } // while(true)
...
void AmRtpAudio::record(unsigned long long system_ts, unsigned char *buffer, int input_sample_rate, unsigned int size)
{
    if (!size)
        return;
    if (mute)
                    ...
                    unsigned int nb_samples)
{
    ...
    nb_samples = static_cast<unsigned int>(static_cast<float>(nb_samples) * static_cast<float>(getSampleRate()) /
                                           static_cast<float>(output_sample_rate));

    u_int32_t size = PCM16_S2B(playout_buffer->read(
        user_ts, reinterpret_cast<ShortSample *>(static_cast<unsigned char *>(samples)), nb_samples));
    ...
    if (output_sample_rate != getSampleRate()) {
        size = resampleOutput(static_cast<unsigned char *>(samples), size, getSampleRate(), output_sample_rate);
    }
...
int AmRtpAudio::put(unsigned long long system_ts, unsigned char *buffer, int input_sample_rate, unsigned int size)
{
    last_send_ts_i = true;
    ...
    size = resampleInput(static_cast<unsigned char *>(samples), size, input_sample_rate, getSampleRate());
    ...
    return send(tx_user_ts, static_cast<unsigned char *>(samples), static_cast<unsigned int>(s));
}
...
int AmRtpAudio::init(const AmSdp &local, const AmSdp &remote, bool sdp_offer_owner, bool force_passive_mode)
{
    ...
    if (local.media[sdp_media_index].type == MT_AUDIO) {
        ...
        if (session && !session->getRtpFrameSize(frame_size))
...
int AmRtpAudio::setCurrentPayload(int payload, int frame_size)
{
    if (payload != this->payload) {
        ...
        CLASS_DBG("change payload %d -> %d, local_ssrc: 0x%x, local_tag: %s", this->payload, payload, l_ssrc,
                  session ? session->getLocalTag().c_str() : "no session");

        PayloadMappingTable::iterator pmt_it = pl_map.find(static_cast<PayloadMappingTable::key_type>(payload));
        if (pmt_it == pl_map.end()) {
            CLASS_DBG("received payload %i is not described in local SDP. ignore it. "
                      "local_ssrc: 0x%x, local_tag: %s",
                      payload, l_ssrc, session ? session->getLocalTag().c_str() : "no session");
        ...
        if (index >= payloads.size()) {
            ERROR("Could not set current payload: payload %i maps to invalid index %i", payload, index);
            last_not_supported_rx_payload = payload;
            wrong_payload_errors++;
            return -1;
        }
        ...
        if (isLocalTelephoneEventPayload(payload)) {
            CLASS_ERROR("Attempt to set telephone-event payload %d as default audio payload. ignore it", payload);
            last_not_supported_rx_payload = payload;
            wrong_payload_errors++;
            return -1;
        }
        ...
        int          ret = static_cast<AmAudioRtpFormat *>(fmt.get())->setCurrentPayload(payloads[index], frame_size);
        if (!ret) {
            amci_codec_t *codec = fmt->getCodec();
            use_default_plc     = ((codec == nullptr) || (codec->plc == nullptr));
...
unsigned int AmRtpAudio::conceal_loss(unsigned int ts_diff, unsigned char *buffer)
{
    ...
    if (!use_default_plc) {
        ...
        s = static_cast<unsigned int>((*codec->plc)(buffer, PCM16_S2B(ts_diff),
                                                    static_cast<unsigned int>(fmt->channels),
                                                    static_cast<unsigned int>(getSampleRate()), fmt->getHCodec()));
        // DBG("codec specific PLC (ts_diff = %i; s = %i)",ts_diff,s);
        return s;
    }

_default_plc:
    s = default_plc(buffer, PCM16_S2B(ts_diff), static_cast<unsigned int>(fmt->channels),
                    static_cast<unsigned int>(getSampleRate()));
    // DBG("default PLC (ts_diff = %i; s = %i)",ts_diff,s);
    return s;
}

unsigned int AmRtpAudio::default_plc(unsigned char *out_buf, unsigned int size, unsigned int /*channels*/,
