...
int trsp_base_input::parse_input(tcp_base_trsp *socket)
{
    ...
    for (int i = 0;; i++) {
        ...
        DBG3("sd:%d, skip_sip_msg_async = %d, st:%d, stage:%d, i:%d", socket->get_sd(), err, pst.st, pst.stage, i);
        ...
        if (err) {
            if (err == UNEXPECTED_EOT) {
                if (pst.orig_buf > (char *)input_buf) {
                    ...
                    DBG("incomplete message. move existent data to the buffer start. sd:%d addr_shift:%d", socket->sd,
                        addr_shift);
                    return 0;
                } else if (trsp_base_input::get_input_free_space()) {
                    DBG("incomplete message at the buffer start. sd:%d", socket->sd);
                    return 0;
                }
                DBG("message is too big. drop connection. sd:%d peer %s:%d", socket->sd, socket->get_peer_ip().data(),
                    socket->get_peer_port());
            } else {
                DBG("parsing error %d. sd:%d peer %s:%d", err, socket->sd, socket->get_peer_ip().data(),
                    socket->get_peer_port());
            }
        ...
        if (msg_len > MAX_TCP_MSGLEN) {
            DBG("message is too big (%d > %d. drop connection. peer %s:%d", msg_len, MAX_TCP_MSGLEN,
        ...
        if (trsp_socket::log_level_raw_msgs >= 0) {
            _LOG(trsp_socket::log_level_raw_msgs,
                 ...
                 "--++--\n%.*s--++--\n",
                 socket->get_transport(), socket->sd, am_inet_ntop_sip(&s_msg->remote_ip, host, NI_MAXHOST),
                 am_get_port(&s_msg->remote_ip), am_inet_ntop_sip(&s_msg->local_ip, host, NI_MAXHOST),
                 am_get_port(&s_msg->local_ip), s_msg->len, s_msg->len, s_msg->buf);
        }
...
tcp_base_trsp::tcp_base_trsp(trsp_server_socket *server_sock_, trsp_worker *server_worker_, int sd,
                             const sockaddr_storage *sa, trsp_socket::socket_transport transport, event_base *evbase_,
                             trsp_input *input_)
    : trsp_socket(server_sock_->get_if(), server_sock_->get_proto_idx(), 0, transport, 0, sd)
    , server_sock(server_sock_)
    , server_worker(server_worker_)
    , input(input_)
    , closed(false)
    , connected(false)
    , evbase(evbase_)
    ...
    , write_ev(NULL)
{
    ...
    CLASS_DBG3("tcp_base_trsp() server_socket:%p transport:%d sa:%s:%i trsp:%d ssl_marker:%d sig:%d cipher:%d mac:%d",
               server_sock, transport, am_inet_ntop(sa).c_str(), am_get_port(sa), sa_ssl->trsp, sa_ssl->ssl_marker,
               sa_ssl->sig, sa_ssl->cipher, sa_ssl->mac);
...
{
    if (read_ev) {
        ERROR("read event already created: transport %s, ip %s, port %d", get_transport(),
              am_inet_ntop(&peer_addr).c_str(), am_get_port(&peer_addr));
    }
    read_ev = event_new(evbase, sd, EV_READ | EV_PERSIST, tcp_base_trsp::on_sock_read, (void *)this);
    DBG3("%p created read_ev %p with base %p", this, read_ev, evbase);

    if (write_ev) {
        ERROR("write event already created: transport %s, ip %s, port %d", get_transport(),
              am_inet_ntop(&peer_addr).c_str(), am_get_port(&peer_addr));
    }
    write_ev = event_new(evbase, sd, EV_WRITE, tcp_base_trsp::on_sock_write, (void *)this);
...
int tcp_base_trsp::connect(int port_from_pool)
{
    ...
    CLASS_DBG3("tcp_base_trsp::connect(): sd:%d ss_family:%d addr:%s:%i trsp:%d ssl_marker:%d sig:%d cipher:%d mac:%d",
               sd, peer_addr.ss_family, am_inet_ntop(&peer_addr).c_str(), am_get_port(&peer_addr), peer_addr_ssl->trsp,
               peer_addr_ssl->ssl_marker, peer_addr_ssl->sig, peer_addr_ssl->cipher, peer_addr_ssl->mac);
    ...
    if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, (void *)&true_opt, sizeof(true_opt)) == -1) {
    ...
    if (socket_options & static_client_port || port_from_pool) {
        ...
        if (setsockopt(sd, SOL_SOCKET, SO_LINGER, (void *)&linger_opt, sizeof(struct linger)) == -1) {
    ...
    DBG("connecting to %s:%i...", am_inet_ntop(&peer_addr).c_str(), am_get_port(&peer_addr));
...
void tcp_base_trsp::on_read(short ev)
{
    ...
    { // locked section
        ...
        DBG3("on_read (sd:%d, connected:%i, transport:%s, input_free_space:%d) read = %d", sd, connected,
             get_transport(), input_free_space, bytes);

        if (bytes < 0) {
            switch (errno) {
            case EAGAIN: return; // nothing to read
...
void tcp_base_trsp::on_write(short ev)
{
    ...
    DBG3("on_write (sd:%d, connected:%i, transport:%s)", sd, connected, get_transport());
    ...
    while (!send_q.empty() && !closed) {
        ...
        if (trsp_socket::log_level_raw_msgs >= 0) {
            _LOG(trsp_socket::log_level_raw_msgs, "sent msg via %s/%i from %s:%i to %s:%i. bytes: %d/%d",
                 get_transport(), sd, actual_ip.c_str(), actual_port, get_addr_str(&msg->addr).c_str(),
                 am_get_port(&msg->addr), bytes, msg->bytes_left());
        }
...
int tcp_base_trsp::on_connect(short ev)
{
    ...
    if (error != 0) {
        DBG("*********** connection error (sd=%i): %s *********", sd, strerror(error));
        close();
        return -1;
    }

    DBG("TCP connection from %s:%u", get_peer_ip().c_str(), get_peer_port());
    set_connected(true);
    add_read_event();
...
tcp_base_trsp::msg_buf::msg_buf(const sockaddr_storage *sa, const char *msg, const int msg_len)
    : msg_len(msg_len)
{
                      ...
                      const int msg_len, unsigned int flags)
{
    ...
    if (sock_it != connections.end()) {
        ...
        if (sa_ssl->ssl_marker ^ peer_ssl->ssl_marker)
            WARN("send/peer addresses ssl markers are not equal: send %d, peer %d", sa_ssl->ssl_marker,
                 peer_ssl->ssl_marker);
    }
...
template <class socket> unsigned long long trsp_worker::getQueueSize()
{
    vector<socket *> conns;
    {
        AmLock l(connections_mut);
...
void trsp_worker::run()
{
    ...
    (void)ret;
    struct event *ev_default = event_new(evbase, fake_fds[0], EV_READ | EV_PERSIST, NULL, NULL);
...
trsp_server_socket::trsp_server_socket(unsigned short if_num, unsigned short proto_idx, unsigned int opts,
...
void trsp_server_socket::add_event(struct event_base *evbase)
{
    ...
    if (!ev_accept) {
        ev_accept = event_new(evbase, sd, EV_READ | EV_PERSIST, trsp_server_socket::on_accept, (void *)this);
...
trsp_statistics::trsp_st_base::trsp_st_base(trsp_socket::socket_transport transport, unsigned short if_num,
                                            unsigned short proto_idx)
    : countOutPendingConnections(
    ...
    , countInPendingConnections(
          stat_group(Gauge, "core", "connections")
              ...
              .addLabel("transport", trsp_socket::socket_transport2proto_str(transport))
              .addLabel("protocol", AmConfig.sip_ifs[if_num].proto_info[proto_idx]->ipTypeToStr()))
    , sipParseErrors(stat_group(Counter, "core", "sip_parse_errors")
                         .addAtomicCounter()
                         .addLabel("interface", AmConfig.sip_ifs[if_num].name)
                         .addLabel("transport", trsp_socket::socket_transport2proto_str(transport))
                         .addLabel("protocol", AmConfig.sip_ifs[if_num].proto_info[proto_idx]->ipTypeToStr()))
