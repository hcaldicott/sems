...
_trans_layer::_trans_layer()
    ...
    , transports()
{
    stat_group(Gauge, "core", "sip_transactions").addFunctionCounter([]() -> unsigned long long {
        return trans_layer::instance()->get_trans_count();
    });
}
...
int _trans_layer::register_transport(trsp_socket *trsp)
{
    ...
    if (transports[if_num].find(trsp->get_transport_id()) != transports[if_num].end()) {
...
void _trans_layer::clear_transports()
{
    for (auto &trans : transports) {
        for (auto &proto : trans)
            dec_ref(proto.second);
    }
    transports.clear();
}

int _trans_layer::set_trsp_socket(sip_msg *msg, const trsp_socket::socket_transport &next_trsp, int out_interface)
{
    ...
    if ((out_interface < 0) || ((unsigned int)out_interface >= transports.size())) {
        ...
        if ((unsigned int)out_interface >= transports.size()) {
            DBG("new out_interface is out of the bounds. out_interface = %d, transports.size() = %lu", out_interface,
                transports.size());
            return -1;
        }
    }
    ...
    prot_collection::iterator prot_sock_it = transports[out_interface].find(next_trsp);
    ...
    DBG("set_trsp_socket: msg->local_socket:%p msg->remote_ip:%s:%i trsp:%d ssl_marker:%d sig:%d cipher:%d mac:%d",
        msg->local_socket, am_inet_ntop(&msg->remote_ip).c_str(), am_get_port(&msg->remote_ip), remote_ip_ssl->trsp,
        remote_ip_ssl->ssl_marker, remote_ip_ssl->sig, remote_ip_ssl->cipher, remote_ip_ssl->mac);

    return 0;
}

static int patch_contact_transport(sip_header *contact, const cstring &trsp, string &n_contact)
{
    DBG("contact: <%.*s>", contact->value.len, contact->value.s);

    list<cstring> contact_list;
    if (parse_nameaddr_list(contact_list, contact->value.s, contact->value.len) < 0) {
    ...
    for (list<cstring>::iterator ct_it = contact_list.begin(); ct_it != contact_list.end(); ct_it++) {
        ...
        for (list<sip_avp *>::iterator p_it = na.uri.params.begin(); p_it != na.uri.params.end(); p_it++) {

            if (!lower_cmp_n((*p_it)->name.s, (*p_it)->name.len, "transport", 9)) {
                found_trsp = true;
                if (lower_cmp_n((*p_it)->value.s, (*p_it)->value.len, trsp.s, trsp.len)) {
...
int _trans_layer::send_reply(sip_msg *msg, const trans_ticket *tt, const cstring &dialog_id, const cstring &to_tag,
                             msg_logger *logger, msg_sensor *sensor)
{
    ...
    if (!local_socket->is_opt_set(trsp_socket::no_transport_in_contact)) {
        ...
        for (list<sip_header *>::iterator contact_it = msg->contacts.begin(); contact_it != msg->contacts.end();
             contact_it++, contact_buf_it++)
        {
    ...
    for (list<sip_header *>::iterator it = req->hdrs.begin(); it != req->hdrs.end(); ++it) {
    ...
    *c++ = LF;
    ...
    DBG("Sending to %s:%i <%.*s...>", get_addr_str(&remote_ip).c_str(), ntohs(((sockaddr_in *)&remote_ip)->sin_port),
    ...
    if (logger || sensor) {
        ...
        if (logger) {
            logger->log(reply_buf, reply_len, &src_ip, &remote_ip, req->u.request->method_str, reply_code);
            if (t->logger) {
...
int _trans_layer::send_sf_error_reply(const trans_ticket *tt, const sip_msg *req, int reply_code, const cstring &reason,
...
int _trans_layer::send_sl_reply(sip_msg *req, int reply_code, const cstring &reason, const cstring &hdrs,
                                const cstring &body)
{
    // Ref.: RFC 3261 8.2.6, 12.1.1
    //
...
//
int _trans_layer::set_next_hop(sip_msg *msg, cstring *next_hop, unsigned short *next_port, cstring *next_trsp,
                               cstring *next_scheme)
{
    ...
    if (!route_hdrs.empty()) {
        ...
        if (err < 0) {
            ERROR("[%.*s] Invalid Request URI: '%.*s'", msg->callid->value.len, msg->callid->value.s, r_uri.len,
                  r_uri.s);
            return -1;
        }
    ...
    DBG("next_scheme:next_hop:next_port is <%.*s:%.*s:%u/%.*s>", next_scheme->len, next_scheme->s, next_hop->len,
        next_hop->s, *next_port, next_trsp ? next_trsp->len : 0, next_trsp ? next_trsp->s : 0);

    return 0;
}

static void set_err_reply_from_req(sip_msg *err, sip_msg *req, int code, const char *reason)
{
    err->type                 = SIP_REPLY;
    ...
    if (!req->from->p) {
        ...
        parse_from_to((sip_from_to *)req->from->p, req->from->value.s, req->from->value.len);
    }
    err->from = req->from;
    ...
    if (!req->to->p) {
        ...
        parse_from_to((sip_from_to *)req->to->p, req->to->value.s, req->to->value.len);
    }
    err->to = req->to;
    ...
    if (!req->cseq->p) {
        ...
        parse_cseq((sip_cseq *)req->cseq->p, req->cseq->value.s, req->cseq->value.len);
    }
    err->cseq         = req->cseq;
...
void _trans_layer::transport_error(sip_msg *msg)
{
    ...
    if ((msg->type != SIP_REQUEST) || (msg->u.request->method == sip_request::ACK))
        return;
...
static void translate_string(sip_msg *dst_msg, cstring &dst, const sip_msg *src_msg, const cstring &src)
{
    dst.s   = (char *)src.s + (dst_msg->buf - src_msg->buf);
    dst.len = src.len;
}

static void translate_hdr(sip_msg *dst_msg, sip_header *&dst, const sip_msg *src_msg, const sip_header *src)
{
    dst = new sip_header();
...
static void gen_error_reply_from_req(sip_msg &reply, const sip_msg *req, int code, const char *reason)
{
    ...
    parse_from_to((sip_from_to *)reply.from->p, reply.from->value.s, reply.from->value.len);

    translate_hdr(&reply, reply.to, req, req->to);
    reply.to->p = new sip_from_to();
    parse_from_to((sip_from_to *)reply.to->p, reply.to->value.s, reply.to->value.len);

    translate_hdr(&reply, reply.cseq, req, req->cseq);
    reply.cseq->p = new sip_cseq();
    parse_cseq((sip_cseq *)reply.cseq->p, reply.cseq->value.s, reply.cseq->value.len);
...
void _trans_layer::get_trans_list(AmArg &ret)
{
    dumps_transactions([&ret](sip_trans *trans) {
        ...
        arg["method"]    = trans->msg->type == SIP_REQUEST
                               ? string(trans->msg->u.request->method_str.s, trans->msg->u.request->method_str.len)
                               : "";
        arg["remote_ip"] = am_inet_ntop(&trans->msg->remote_ip);
        struct timeval tv;
        gettimeofday(&tv, 0);
        arg["age"]        = (long long)((tv.tv_sec * 1000 + tv.tv_usec / 1000) -
                                 (trans->time_created.tv_sec * 1000 + trans->time_created.tv_usec / 1000));
...
static int patch_ruri_with_remote_ip(string &n_uri, sip_msg *msg)
{
    ...
    if (parsed_uri.port_str.len) {
        ...
        n_uri += string(parsed_uri.port_str.s + parsed_uri.port_str.len,
                        old_ruri.s + old_ruri.len - (parsed_uri.port_str.s + parsed_uri.port_str.len));
    } else {
        // copy from end of host-string until the end of old R-URI
        n_uri += string(parsed_uri.host.s + parsed_uri.host.len,
                        old_ruri.s + old_ruri.len - (parsed_uri.host.s + parsed_uri.host.len));
    }
...
int _trans_layer::send_request(sip_msg *msg, trans_ticket *tt, const cstring &dialog_id, const cstring &_next_hop,
                               int out_interface, unsigned int flags, msg_logger *logger, msg_sensor *sensor,
                               sip_timers_override *timers_override, sip_target_set *targets_, int redirects_allowed)
{
    // Request-URI
    // To
    // From
    ...
    if (_next_hop.len) {
        ...
        if (res || dest_list.empty()) {
            DBG("parse_next_hop %.*s failed (%i)", _next_hop.len, _next_hop.s, res);
            return res;
        }
    } else {
...
try_next_dest:
    ...
    if ((flags & TR_FLAG_NEXT_HOP_RURI) && (patch_ruri_with_remote_ip(ruri, msg) < 0)) {
        return -1;
    }
    ...
    tt->_bucket = get_trans_bucket(p_msg->callid->value, get_cseq(p_msg)->num_str);
    tt->_bucket->lock();
    ...
    if (err < 0) {
        ...
        err = update_uac_request(tt->_bucket, tt->_t, p_msg, timers_override, redirects_allowed);
        if (err < 0) {
        ...
        if (tt->_t && (method != sip_request::ACK)) {
            ...
            if (tt->_t->targets)
                delete tt->_t->targets;
            tt->_t->targets = targets.release();

            if (tt->_t->targets->has_next()) {
                tt->_t->timer_m = timers_override && timers_override->stimer_m ? timers_override->stimer_m : M_TIMER;
                tt->_t->reset_timer(STIMER_M, tt->_t->timer_m, tt->_bucket->get_id());
            }
        }
        ...
        if (logger || sensor) {
            ...
            if (sensor) {
                sensor->feed(msg_buffer, msg_len, &src_ip, &msg->remote_ip, msg_sensor::PTYPE_SIP);
...
int _trans_layer::cancel(trans_ticket *tt, const cstring &dialog_id, unsigned int inv_cseq, unsigned int maxf,
                         const cstring &hdrs, const cstring &dlg_route_set_hdrs)
{
    ...
    if (req->u.request->method != sip_request::INVITE) {
        t->dump();
        bucket->unlock();
        ERROR("Trying to cancel a non-INVITE request (we SHOULD NOT do that); inv_cseq: %u, i:%.*s", inv_cseq,
    ...
    case TS_COMPLETED:
        ERROR("Trying to cancel a request while in TS_COMPLETED state; inv_cseq: %u, i:%.*s", inv_cseq, dialog_id.len,
              dialog_id.s);
        t->dump();
        bucket->unlock();
        return -1;
    ...
    default:
        ERROR("Trying to cancel a request while in %s state; inv_cseq: %u, i:%.*s", t->state_str(), inv_cseq,
              dialog_id.len, dialog_id.s);
        t->dump();
        bucket->unlock();
        return -1;
    ...
    int request_len = sip_request_line_len(cancel_str, req->u.request->ruri_str);

    request_len += copy_hdr_len(req->via1);

    request_len += copy_hdr_len(req->to) + copy_hdr_len(req->from) + copy_hdr_len(req->callid) +
                   cseq_len(get_cseq(req)->num_str, cancel_str) + dlg_route_set_hdrs.len +
                   copy_hdrs_len(req->contacts) + copy_hdr_len(&maxfrwd);
    ...
    // generate it
    char *c = p_msg->buf;
    sip_request_line_wr(&c, cancel_str, req->u.request->ruri_str);
    ...
    *c   = '\0';
    ...
    DBG("Sending to %s:%i:\n<%.*s>", get_addr_str(&p_msg->remote_ip).c_str(),
        ntohs(((sockaddr_in *)&p_msg->remote_ip)->sin_port), p_msg->len, p_msg->buf);
    ...
    if (send_err < 0) {
        ...
        if (send_err < 0) {
            ...
            if (t->logger || t->sensor) {
                ...
                if (t->logger) {
                    t->logger->log(p_msg->buf, p_msg->len, &src_ip, &p_msg->remote_ip, cancel_str);
                ...
                if (t->sensor) {
                    t->sensor->feed(p_msg->buf, p_msg->len, &src_ip, &p_msg->remote_ip, msg_sensor::PTYPE_SIP);
...
void _trans_layer::received_msg(sip_msg *msg, const trsp_acls &acls)
{
    ...
    if (err) {
        ...
        if ((err != MALFORMED_FLINE) && (msg->type == SIP_REQUEST) && (msg->u.request->method != sip_request::ACK)) {
...
void _trans_layer::process_rcvd_msg(sip_msg *msg, const trsp_acls &acls)
{
    ...
    case SIP_REQUEST:
        ...
        if ((t = bucket->match_request(msg, TT_UAS)) != NULL) {
            ...
            if (t->logger) {
                t->logger->log(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip, msg->u.request->method_str);
            }
            if (t->sensor) {
                t->sensor->feed(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip, msg_sensor::PTYPE_SIP);
            }
            ...
            case sip_request::PRACK:
                bucket->unlock();
                if (!msg->rack) {
                    send_sl_reply(msg, 400, cstring("Missing valid RSeq header"), cstring(), cstring());
                    DROP_MSG;
                }
            ...
            default:
                ...
                case trsp_acl::Reject:
                    ...
                    send_sl_reply(msg, 403, sip_resp_forbidden, cstring(), cstring());
                    return;
                    break;
                }
    ...
    case SIP_REPLY:
        ...
        if ((t = bucket->match_reply(msg)) != NULL) {
            ...
            if (t->logger) {
                t->logger->log(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip, get_cseq(msg)->method_str,
                               msg->u.reply->code);
            }
            if (t->sensor) {
                t->sensor->feed(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip, msg_sensor::PTYPE_SIP);
    ...
    default: ERROR("Got unknown message type: Bug?"); break;
    }
...
int _trans_layer::update_uac_reply(trans_bucket *bucket, sip_trans *t, sip_msg *&msg)
{
    ...
    if (reply_code < 200) {
        ...
        case TS_ABANDONED:
            ...
            INFO("disable blacklisting: remote UA (%s/%i) did reply", am_inet_ntop(&msg->remote_ip).c_str(),
    ...
    if (t->msg->u.request->method == sip_request::INVITE) {
        ...
        if (reply_code >= 300) {
            ...
            if (t->state == TS_CALLING || t->state == TS_PROCEEDING) {
                if (reply_code == 503) {
                ...
                } else if (msg->local_socket && (reply_code == 302 || reply_code == 301)) {
            ...
            case TS_TERMINATED_200: // subsequent 2xx reply (no ACK sent)
                ...
                if ((to_tag.len != t->to_tag.len) || (memcmp(to_tag.s, t->to_tag.s, to_tag.len) != 0)) {
        ...
        case TS_TERMINATED:
            ...
            DBG("disable blacklisting: remote UA (%s/%i) did reply", am_inet_ntop(&msg->remote_ip).c_str(),
                am_get_port(&msg->remote_ip));
            t->flags |= TR_FLAG_DISABLE_BL;
            goto end;
...
int _trans_layer::update_uac_request(trans_bucket *bucket, sip_trans *&t, sip_msg *msg,
                                     sip_timers_override *timers_override, int redirects_allowed)
{
    ...
    DBG("transport = '%s'; is_reliable = %i", msg->local_socket->get_transport(), msg->local_socket->is_reliable());
...
int _trans_layer::update_uas_request(trans_bucket *bucket, sip_trans *t, sip_msg *msg)
{
    ...
    default: DBG("Bug? Unknown state at this point: %i", t->state); break;
    }
...
void _trans_layer::send_non_200_ack(sip_msg *reply, sip_trans *t)
{
    ...
    ack_len += copy_hdr_len(inv->via1) + copy_hdr_len(inv->from) + copy_hdr_len(reply->to) + copy_hdr_len(inv->callid) +
...
void _trans_layer::timer_expired(trans_timer *t, trans_bucket *bucket, sip_trans *tr)
{
    int n    = t->type >> 16;
    ...
    case STIMER_A: // Calling: (re-)send INV
        ...
        if (tr->logger || tr->sensor) {
            ...
            if (tr->sensor) {
                tr->sensor->feed(tr->msg->buf, tr->msg->len, &src_ip, &tr->msg->remote_ip, msg_sensor::PTYPE_SIP);
            }
        }
    ...
    case STIMER_B: // Calling: -> Terminated
        ...
        if (tr->state == TS_CALLING) {
            ...
            DBG("Transaction timeout timer hit while state=%s (0x%x)", tr->state_str(), tr->state);
            bucket->remove(tr);
        }
        break;
    ...
    { // Completed: (re-)send response
        ...
        unsigned int retr_timer = (type == STIMER_E) ? E_TIMER << n : G_TIMER << n;
    ...
    default: ERROR("Invalid timer type %i", type); break;
    }
...
int _trans_layer::find_outbound_if(sockaddr_storage *remote_ip)
{
    ...
    if (am_inet_ntop(&from, local_ip, NI_MAXHOST) != NULL) {
        map<string, unsigned short>::iterator if_it = AmConfig.local_sip_ip2if.find(local_ip);
...
int _trans_layer::try_next_ip(trans_bucket *bucket, sip_trans *tr, bool use_new_trans)
{
...
try_next_dest:
    ...
    if (use_new_trans) {
        ...
        if (tr->flags & TR_FLAG_NEXT_HOP_RURI) {
            ...
            compute_branch((char *)(tr->msg->via_p1->branch.s + MAGIC_BRANCH_LEN), tr->msg->callid->value,
                           tr->msg->cseq->value);
        }
    ...
    if (tr->logger || tr->sensor) {
        ...
        tr->msg->local_socket->copy_addr_to(&src_ip);
        if (tr->logger) {
            tr->logger->log(tr->msg->buf, tr->msg->len, &src_ip, &tr->msg->remote_ip, tr->msg->u.request->method_str);
        }
        if (tr->sensor) {
            tr->sensor->feed(tr->msg->buf, tr->msg->len, &src_ip, &tr->msg->remote_ip, msg_sensor::PTYPE_SIP);
        }
...
int _trans_layer::retarget(sip_trans *t, sip_msg *&msg, std::unique_ptr<sip_trans> &new_tr)
{
    int res = 0;
    ...
    if (NULL == comma_pos)
        contact_len = reply_contact->value.len;
    else
        contact_len = comma_pos - c;

    if (parse_nameaddr_uri(&contact_nameaddr, &c, contact_len)) {
        DBG("retarget: failed to parse first reply contact: '%.*s'", reply_contact->value.len, reply_contact->value.s);
    ...
    if (parse_uri(&parsed_r_uri, old_ruri.s, old_ruri.len) < 0) {
        DBG("retarget: could not parse local R-URI ('%.*s')", old_ruri.len, old_ruri.s);
        update_reply_msg(500, "Failed to parse request r-uri");
        return -1;
    }

    // apply contact to ruri
    string n_uri = string(old_ruri.s, parsed_r_uri.user.s - old_ruri.s);
    if (parsed_contact.user.len) {
        n_uri += string(parsed_contact.user.s, parsed_contact.user.len) + '@';
    }
    n_uri += string(parsed_contact.host.s, parsed_contact.host.len);
    if (parsed_contact.port != 5060) {
        n_uri += ":" + string(parsed_contact.port_str.s, parsed_contact.port_str.len);
    }
    if (parsed_contact.trsp) {
        n_uri += ";" + string(parsed_contact.trsp->name.s, parsed_contact.trsp->name.len) + "=" +
                 string(parsed_contact.trsp->value.s, parsed_contact.trsp->value.len);
    }

    sip_msg tmp_msg(*t->msg);
    if (tmp_msg.local_socket)
        inc_ref(tmp_msg.local_socket);
    tmp_msg.vias.pop_front();
    ...
    DBG("retarget: '%.*s' -> '%s'", old_ruri.len, old_ruri.s, n_uri.c_str());
...
void trans_ticket::assign_if_differs(const trans_ticket &other)
{
    ...
    if (_bucket->exist(_t)) {
        ...
        if (other._bucket->exist(other._t) && (_t != other._t)) {
            DBG("patch ticket transaction %p -> %p", _t, other._t);
            *this = other;
        }
