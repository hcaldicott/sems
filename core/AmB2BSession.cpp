...
static bool connectionActive(const SdpConnection &connection, bool default_value)
{
    ...
    if (connection.address == zero_connection)
        return false;
    return true;
}
...
static MediaActivity getMediaActivity(const vector<SdpAttribute> &attrs, MediaActivity default_value)
{
    ...
    for (std::vector<SdpAttribute>::const_iterator a = attrs.begin(); a != attrs.end(); ++a) {
        if (a->attribute == sendonly)
            return Sendonly;
        if (a->attribute == inactive)
            return Inactive;
        if (a->attribute == recvonly)
            return Recvonly;
        if (a->attribute == sendrecv)
            return Sendrecv;
    }
...
AmB2BSession::AmB2BSession(const string &other_local_tag, AmSipDialog *p_dlg, AmSipSubscription *p_subs)
    : AmSession(p_dlg)
    ...
    , rtp_ping(false)
    , rtp_relay_transparent_seqno(false)
    , rtp_relay_transparent_ssrc(false)
...
{
    if (!subs)
        subs = new AmSipSubscription(dlg, this);
}
...
void AmB2BSession::relayError(const string &method, unsigned cseq, bool forward, int err_code)
{
    if (method != "ACK") {
        ...
        if (n_reply.code == 491) {
            ...
            n_reply.hdrs += SIP_HDR_COLSP(SIP_HDR_RETRY_AFTER) + int2str((get_random() % 8) + 2) + CRLF;
        }
...
void AmB2BSession::onB2BEvent(B2BEvent *ev)
{
    ...
    case B2BSipRequest:
    {
        ...
        DBG("B2BSipRequest: %s (fwd=%s)", req_ev->req.method.c_str(), req_ev->forward ? "true" : "false");
    ...
    case B2BSipReply:
    {
        ...
        DBG("B2BSipReply: %i %s (fwd=%s)", reply_ev->reply.code, reply_ev->reply.reason.c_str(),
            reply_ev->forward ? "true" : "false");

        DBG("B2BSipReply: content-type = %s", reply_ev->reply.body.getCTStr().c_str());

        if (reply_ev->forward) {
            std::map<int, AmSipRequest>::iterator t_req = recvd_req.find(reply_ev->reply.cseq);
            if (t_req != recvd_req.end()) {
                if ((reply_ev->reply.code >= 300) && (reply_ev->reply.code <= 305) && !reply_ev->reply.contact.empty())
                {
            ...
            if (SIP_IS_200_CLASS(reply_ev->reply.code) && (!reply_ev->reply.body.empty()) &&
                (reply_ev->reply.cseq_method == SIP_METH_INVITE || reply_ev->reply.cseq_method == SIP_METH_UPDATE))
            {
...
bool AmB2BSession::getMappedReferID(unsigned int refer_id, unsigned int &mapped_id) const
{
    map<unsigned int, unsigned int>::const_iterator id_it = refer_id_map.find(refer_id);
...
void AmB2BSession::insertMappedReferID(unsigned int refer_id, unsigned int mapped_id)
{
...
{
    bool fwd = sip_relay_only && (req.method != SIP_METH_CANCEL) && (req.method != SIP_METH_INFO);

    if (((req.method == SIP_METH_SUBSCRIBE) || (req.method == SIP_METH_NOTIFY) || (req.method == SIP_METH_REFER)) &&
    ...
    if (fwd) {
        DBG("relaying B2B SIP request (fwd) %s %s", r_ev->req.method.c_str(), r_ev->req.r_uri.c_str());
            ...
            if (event == "refer" && !id.empty()) {
                ...
                if (str2int(id, id_int)) {
                    ...
                    if (getMappedReferID(id_int, mapped_id)) {
                        ...
                        r_ev->req.hdrs += SIP_HDR_COLSP(SIP_HDR_EVENT) "refer;id=" + int2str(mapped_id) + CRLF;
                    }
                }
            }
        }
    ...
    if (r_ev->req.method != SIP_METH_INFO) {
        DBG("relaying B2B SIP request %s %s", r_ev->req.method.c_str(), r_ev->req.r_uri.c_str());
        relayEvent(r_ev);
    }
}
...
void AmB2BSession::onSipReply(const AmSipRequest &req, const AmSipReply &reply, AmBasicSipDialog::Status old_dlg_status)
{
    TransMap::iterator t   = relayed_req.find(reply.cseq);
    bool               fwd = (t != relayed_req.end()) && (reply.code != 100);

    DBG("onSipReply: %s -> %i %s (fwd=%s), c-t=%s", reply.cseq_method.c_str(), reply.code, reply.reason.c_str(),
    ...
    if (!dlg->getRemoteTag().empty() && dlg->getRemoteTag() != reply.to_tag) {
        DBG("sess %p received %i reply with != to-tag: %s (remote-tag:%s)", this, reply.code, reply.to_tag.c_str(),
            dlg->getRemoteTag().c_str());
        return; // drop packet
    }

    if (((reply.cseq_method == SIP_METH_SUBSCRIBE) || (reply.cseq_method == SIP_METH_NOTIFY) ||
...
{
    CLASS_DBG("AmB2BSession::onSdpCompleted(). %s SDP offer owner", sdp_offer_owner ? "local" : "remote");
    ...
    if (isHoldRequest(remote_sdp, method)) {
        if (!remote_on_hold) {
            DBG("[%s] got hold request. method: %d", getLocalTag().c_str(), method);
            remote_on_hold = true;
            onHoldRequest();
        }
    } else if (remote_on_hold) {
        DBG("[%s] got resume request. method: %d", getLocalTag().c_str(), method);
        remote_on_hold = false;
        onResumeRequest();
    }
...
bool AmB2BSession::saveSessionDescription(const AmMimeBody &body)
{
    ...
    DBG("saving session description (%s, %.*s...)", sdp_body->getCTStr().c_str(), 50, sdp_body->getPayload());
...
bool AmB2BSession::updateSessionDescription(const AmMimeBody &body)
{
    ...
    if (body_hash != new_body_hash) {
        DBG("session description changed - saving (%s, %.*s...)", sdp_body->getCTStr().c_str(), 50,
            sdp_body->getPayload());
...
int AmB2BSession::relaySip(const AmSipRequest &req)
{
    ...
    if (req.method != "ACK") {
        ...
        if (req.method == SIP_METH_PRACK && req.rseq) {
            ...
            for (t = relayed_req.begin(); t != relayed_req.end(); t++) {
                if (t->second.cseq == req.rack_cseq) {
                    m_hdrs = req.hdrs + SIP_HDR_COLSP(SIP_HDR_RACK) + int2str(req.rseq) + " " + int2str(t->first) +
                             " " + req.rack_method + CRLF;
                    hdrs = &m_hdrs;
                    break;
                }
            }
            ...
            if (t == relayed_req.end()) {
                WARN("Transaction with CSeq %d not found for translating RAck cseq, local_tag = %s", req.rack_cseq,
        ...
        if (err < 0) {
            ERROR("dlg->sendRequest() failed, local_tag = %s", dlg->getLocalTag().c_str());
            return err;
        }

        if ((req.method == SIP_METH_INVITE || req.method == SIP_METH_UPDATE) && !req.body.empty()) {
...
int AmB2BSession::relaySip(const AmSipRequest &orig, const AmSipReply &reply)
{
    ...
    if (reply.rseq != 0) {
        m_hdrs = reply.hdrs + SIP_HDR_COLSP(SIP_HDR_RSEQ) + int2str(reply.rseq) + CRLF;
        hdrs   = &m_hdrs;
    }

    AmMimeBody body(reply.body);
    if ((orig.method == SIP_METH_INVITE || orig.method == SIP_METH_UPDATE || orig.method == SIP_METH_ACK ||
    ...
    if ((method == SIP_METH_INVITE || method == SIP_METH_UPDATE) && !reply.body.empty()) {
...
{
    DBG("[%s] set RTP relay mode %d", getLocalTag().data(), mode);
    rtp_relay_mode = mode;
}
...
void AmB2BSession::computeRelayMask(const SdpMedia &m, bool &enable, PayloadMask &mask, PayloadRelayMap &map)
{
    ...
    for (const auto &local_payload : m.payloads) {
        ...
        for (auto const &other_remote_media : other_remote_sdp.media) {
            if (other_remote_media.type != m.type)
                continue; // TODO: use explicit stream index of the other's remote media
            for (const auto &other_remote_payload : other_remote_media.payloads) {
                DBG("other_remote_payload = %s %d", other_remote_payload.encoding_name.c_str(),
                    other_remote_payload.payload_type);
...
void AmB2BCallerSession::onB2BEvent(B2BEvent *ev)
{
    ...
    if (ev->event_id == B2BSipReply) {
        ...
        if (getOtherId().empty()) {
            ...
            DBG("Dialog mismatch! (oi=%s;ft=%s)", getOtherId().c_str(), reply.from_tag.c_str());
            return;
        }
        ...
        case Ringing:
            if (reply.cseq == invite_req.cseq) {
                if (reply.code < 200) {
                    if ((!sip_relay_only) && sip_relay_early_media_sdp && reply.code >= 180 && reply.code <= 183 &&
                        (!reply.body.empty()))
                    {
            ...
            break;
        default: DBG("reply from callee: %u %s", reply.code, reply.reason.c_str()); break;
        } // switch(callee_status)
    } // if(ev->event_id == B2BSipReply)
...
void AmB2BCallerSession::connectCallee(const string &remote_party, const string &remote_uri, bool relayed_invite)
{
...
{
    return dlg->sendRequest(SIP_METH_INVITE, &callee_reply.body, "" /* hdrs */, SIP_FLAGS_VERBATIM);
}
...
void AmB2BCallerSession::createCalleeSession()
{
    ...
    if (AmConfig.log_sessions) {
        INFO("Starting B2B callee session %s", callee_session->getLocalTag().c_str());
    }

    MONITORING_LOG4(getOtherId().c_str(), "dir", "out", "from", callee_dlg->getLocalParty().c_str(), "to",
...
void AmB2BSession::setMediaSession(AmB2BMedia *new_session)
{
    ...
    if (media_session)
        ERROR("BUG: non-empty media session overwritten");
    media_session = new_session;
    if (media_session)
        media_session->addReference(); // new reference for me
    DBG("[%s|%s] B2B media session was set to %p", getLocalTag().c_str(), getOtherId().c_str(), media_session);
}
...
void AmB2BCalleeSession::onB2BEvent(B2BEvent *ev)
{
    if (ev->event_id == B2BConnectLeg) {
        ...
        MONITORING_LOG3(getLocalTag().c_str(), "b2b_leg", getOtherId().c_str(), "to", co_ev->remote_party.c_str(),
...
bool isHoldRequest(const AmSdp &sdp, HoldMethod &method)
{
    ...
    for (std::vector<SdpMedia>::const_iterator m = sdp.media.begin(); m != sdp.media.end(); ++m) {
        ...
        case Recvonly:               // ?
        case Sendrecv: return false; // media stream is active
        }
    }
