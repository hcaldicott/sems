...
static const char *ProcessingStatusStr[] = { "ProcessingEvents", "WaitingDisconnected", "EndedDisconnected" };
...
{
    CLASS_DBG("add stereo recorder. recorder_id: %s, channel_id: %d", recorder_id.c_str(), channel_id);

    stereo_recorders.add(recorder_id.empty() ? getLocalTag() : recorder_id, channel_id);
...
{
    CLASS_DBG("remove stereo recorder. recorder_id: %s, channel_id: %d", recorder_id.c_str(), channel_id);

    stereo_recorders.del(recorder_id.empty() ? getLocalTag() : recorder_id, channel_id);
...
{
    AmSessionProcessorThread *processor_thread = AmSessionProcessor::getProcessorThread(start_on_same_thread);
...
bool AmSession::processingCycle(EventStats *stats)
{
    ...
    DBG("vv S [%s|%s] %s, %s, %i UACTransPending, %i usages vv", dlg->getCallid().c_str(), getLocalTag().c_str(),
        dlg->getStatusStr(), sess_stopped.get() ? "stopped" : "running", dlg->getUACTransPending(), dlg->getUsages());
    ...
    case SESSION_PROCESSING_EVENTS:
    {
        ...
        DBG("^^ S [%s|%s] %s, %s, %i UACTransPending, %i usages ^^", dlg->getCallid().c_str(), getLocalTag().c_str(),
            AmBasicSipDialog::getStatusStr(dlg_status), s_stopped ? "stopped" : "running", dlg->getUACTransPending(),
            dlg->getUsages());
        ...
        if ((dlg_status != AmSipDialog::Disconnected) && (dlg_status != AmSipDialog::Cancelling) && !no_reply) {
            ...
            if (s_stopped && (dlg->getStatus() == AmSipDialog::Disconnected)) {
                processing_status = SESSION_ENDED_DISCONNECTED;
                return false;
            }
        }
    ...
    case SESSION_WAITING_DISCONNECTED:
    {
        ...
        DBG("^^ S [%s|%s] %s, %s, %i UACTransPending, %i usages ^^", dlg->getCallid().c_str(), getLocalTag().c_str(),
            dlg->getStatusStr(), sess_stopped.get() ? "stopped" : "running", dlg->getUACTransPending(),
            dlg->getUsages());
...
void AmSession::session_stopped()
{
    ...
    // current session number
    session_num = session_num - 1;
    if (AmConfig.shutdown_mode && terminate_on_no_sessions && !session_num) {
...
{
    DBG("AmSession::postDtmfEvent(evt: { class: %s, event_id: %d, event: %d, duration: %d})", typeid(*evt).name(),
        evt->event_id, evt->event(), evt->duration());
...
void AmSession::process(AmEvent *ev)
{
    ...
    if (dtmf_ev) {
        DBG3("Session received DTMF, event = %d, duration = %d", dtmf_ev->event(), dtmf_ev->duration());
        onDtmf(dtmf_ev);
        return;
    }
    ...
    if (req.method == SIP_METH_INVITE) {
        ...
        try {
            ...
            if (!no_reply)
                ...
                DBG("AmSession::onSipREquest() suspress reply with reason '%s'", e.reason.c_str());
        }
        ...
        if (supported) {
            string dtmf_body_str((const char *)dtmf_body->getPayload(), dtmf_body->getLen());
...
void AmSession::onSipReply(const AmSipRequest &req, const AmSipReply &reply, AmBasicSipDialog::Status old_dlg_status)
{
    ...
    if (old_dlg_status != dlg->getStatus()) {
        DBG("Dialog status changed %s -> %s (stopped=%s) ", AmBasicSipDialog::getStatusStr(old_dlg_status),
            dlg->getStatusStr(), sess_stopped.get() ? "true" : "false");
    } else {
        DBG("Dialog status stays %s (stopped=%s)", AmBasicSipDialog::getStatusStr(old_dlg_status),
...
int AmSession::onSdpCompleted(const AmSdp &local_sdp, const AmSdp &remote_sdp, bool sdp_offer_owner)
{
    ...
    if (!remote_sdp.media.empty()) {
      vector<SdpAttribute>::const_iterator pos =
        std::find(remote_sdp.media[0].attributes.begin(), remote_sdp.media[0].attributes.end(),
...
int AmSession::getRtpProtoId()
{
    if (rtp_proto_id < 0) {
        ...
        int rtp_proto_id = AmConfig.media_ifs[rtp_if].findProto(dlg->getOutboundAddrType(), MEDIA_info::RTP);
        if (rtp_proto_id >= 0) {
