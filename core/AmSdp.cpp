...
{
    return network == other.network && addrType == other.addrType && address == other.address;
}

bool SdpOrigin::operator==(const SdpOrigin &other) const
{
    return user == other.user && sessId == other.sessId && sessV == other.sessV && conn == other.conn;
}
...
{
    string payload_list;
    for (std::vector<SdpPayload>::const_iterator it = payloads.begin(); it != payloads.end(); it++) {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        case ADDR_TYPE:
            if (src[i] == ' ')
                ...
                addrtype += src[i];
            break;
        case COMMON_ADDR: address += src[i]; break;
        }
    }
    return s == PORT || (s == COMMON_ADDR && !address.empty());
...
int AmSdp::parse(const char *_sdp_msg)
{
    ...
    if (!ret && conn.address.empty()) {
        for (vector<SdpMedia>::iterator it = media.begin(); !ret && (it != media.end()); ++it) {
    ...
    for (vector<SdpMedia>::iterator it = media.begin(); !ret && (it != media.end()); ++it) {
        ...
        if (it->setup == S_UNDEFINED && setup != S_UNDEFINED)
            it->setup = setup;
    }
...
void AmSdp::print(string &body) const
{
    string out_buf = "v=" + int2str(version) +
                     "\r\n"
                     "o=" +
                     origin.user + " " + std::to_string(origin.sessId) + " " + std::to_string(origin.sessV) + " IN ";
    ...
    for (std::vector<SdpMedia>::const_iterator media_it = media.begin(); media_it != media.end(); media_it++) {
        out_buf += "m=" + media_t_2_str(media_it->type) + " " + int2str(media_it->port) + " " +
                   transport_p_2_str(media_it->transport);

        string options;

        auto is_t38_transport = media_it->transport == TP_UDPTL || media_it->transport == TP_UDPTLSUDPTL;

        if (media_it->transport == TP_RTPAVP || media_it->transport == TP_RTPAVPF ||
            media_it->transport == TP_RTPSAVP || media_it->transport == TP_RTPSAVPF ||
            media_it->transport == TP_UDPTLSRTPSAVP || media_it->transport == TP_UDPTLSRTPSAVPF)
        {
                 ...
                 pl_it != media_it->payloads.end(); pl_it++)
            {
                ...
                if (!pl_it->encoding_name.empty()) {
                    options += "a=rtpmap:" + int2str(pl_it->payload_type) + " " + pl_it->encoding_name + "/" +
                               int2str(pl_it->clock_rate);
        ...
        if (!media_it->conn.address.empty())
            out_buf += "\r\nc=IN " + addr_t_2_str(media_it->conn.addrType) + " " + media_it->conn.address;
        ...
        if (media_it->is_ice) {
            ...
            for (std::vector<SdpIceCandidate>::const_iterator c_it = media_it->ice_candidate.begin();
                 c_it != media_it->ice_candidate.end(); c_it++)
            {
                out_buf += c_it->print();
            }
        }
...
void SdpMedia::calcAnswer(const AmPayloadProvider *payload_prov, SdpMedia &answer) const
{
    answer.recv = send;
    answer.send = recv;
    ...
    for (; it != payloads.end(); ++it) {
        ...
        if (a_pl) {
            answer.payloads.push_back(SdpPayload(a_pl->payload_id, a_pl->name, a_pl->advertised_sample_rate, 0));
        } else {
            // Try dynamic payloads
            // and give a chance to broken
            // implementation using a static payload number
            // for dynamic ones.
            int int_pt = payload_prov->getDynPayload(it->encoding_name, it->clock_rate, it->encoding_param);
            if (int_pt != -1) {
...
{
    if (!s)
        return true; // SDP can't be empty, return error (true really used for failure?)
    ...
    while (*s != '\0') {
        ...
        case SDP_DESCR:
            ...
            default:
            {
                ...
                next = skip_till_next_line(s, line_len);
                if (line_len) {
                    DBG("parse_sdp_line: skipping unknown Session description %s=", string(s, line_len).c_str());
                }
                s = next;
                break;
            }
        ...
        case SDP_MEDIA:
            ...
            default:
            {
                next = skip_till_next_line(s, line_len);
                if (line_len) {
                    DBG("parse_sdp_line: skipping unknown Media description '%.*s'", static_cast<int>(line_len), s);
                }
                s = next;
                break;
            }
...
static char *parse_sdp_connection(AmSdp *sdp_msg, char *s, char t)
{
    ...
    if (line_len <= 7) { // should be at least c=IN IP4 ...
        DBG("short connection line '%.*s'", static_cast<int>(line_len), s);
        return next_line;
    }
...
static void parse_sdp_media(AmSdp *sdp_msg, char *s)
{
    ...
    while (parsing) {
        ...
        case FMT:
        {
            if (m.transport == TP_RTPAVP || m.transport == TP_RTPAVPF || m.transport == TP_RTPSAVP ||
                m.transport == TP_RTPSAVPF || m.transport == TP_UDPTLSRTPSAVP || m.transport == TP_UDPTLSRTPSAVPF)
            {
...
static void parse_session_attr(AmSdp *sdp_msg, char *s, char **next)
{
    ...
    *next           = skip_till_next_line(s, line_len);
    ...
    if (col == attr_end) {
        ...
        SdpAttribute a(string(s, static_cast<size_t>(col - s) - 1),
...
static char *parse_sdp_attr(AmSdp *sdp_msg, char *s)
{
    ...
    if (attr == "rtpmap") {
        ...
        for (pl_it = media.payloads.begin();
             (pl_it != media.payloads.end()) && (pl_it->payload_type != int(payload_type)); ++pl_it)
            ;

        if (pl_it != media.payloads.end()) {
            *pl_it = SdpPayload(int(payload_type), encoding_name, int(clock_rate), int(encoding_param));
        }
            ...
            case KEY:
            {
                ...
                if (key_data < line_end) {
                    ...
                    if (method == "inline") {
                        ...
                        DBG("key %s-%d, lifetime %s-%d, mki %d-%d", info.key.c_str(), info.key.size(),
                            info.lifetime.c_str(), info.lifetime.size(), info.mki.id, info.mki.len);
                        break;
                    }
                    break;
                }
...
static char *parse_ice_candidate(SdpMedia *media, char *s)
{
    ...
    while (parsing) {
        ...
        case TRANSPORT:
            ...
            } else if (data.length() == 7 && strncasecmp(data.c_str(), "tcp-act", 7) == 0) {
    ...
    if (parsing) {
        ...
        DBG("SDP: got ice candidate: type %s via %s %s", ice_candidate_2_str(candidate.type).c_str(),
            transport_ice_2_str(candidate.transport).c_str(), candidate.conn.debugPrint().c_str());
    }
    return line_end;
}
...
static void parse_sdp_origin(AmSdp *sdp_msg, char *s)
{
    ...
    while (parsing) {
        ...
        case ID:
        {
            ...
            origin.sessId = strtoul(id.c_str(), 0, 10);
            origin_line   = next;
