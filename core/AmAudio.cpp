...
{
    return (codec && r.codec && (r.codec->id == codec->id) && (r.bytes2samples(1024) == bytes2samples(1024)) &&
    ...
    if (resample_state) {
        ...
        if (src_err) {
            ...
            if (resample_out_buf_samples > out_samples) {
                ...
                memmove(resample_out, &resample_out[out_samples], resample_out_buf_samples * sizeof(float));
            } else {
                resample_out_buf_samples = 0;
            }
        }
...
{
    rstate =
        ResampleFactory::createResampleObj(true, 4.0, ResampleFactory::INTERPOL_SINC, ResampleFactory::SAMPLE_MONO);
}
...
int AmAudio::get(unsigned long long system_ts, unsigned char *buffer, int output_sample_rate, unsigned int nb_samples)
{
    int size = calcBytesToRead((int)((float)nb_samples * (float)getSampleRate() / (float)output_sample_rate));
    ...
    size = resampleOutput((unsigned char *)samples, size, getSampleRate(), output_sample_rate);
...
int AmAudio::put(unsigned long long system_ts, unsigned char *buffer, int input_sample_rate, unsigned int size)
{
    if (!size) {
        return 0;
    }
    ...
    size = resampleInput((unsigned char *)samples, size, input_sample_rate, getSampleRate());
...
int AmAudio::decode(unsigned int size)
{
    ...
    if (out_size > AUDIO_BUFFER_SIZE) {
        WARN("pre-calculated buffer size for pcm16 (%u) bigger than allowed (%u)", out_size, AUDIO_BUFFER_SIZE);
        return -1;
    }
...
unsigned int AmAudio::resampleInput(unsigned char *buffer, unsigned int s, int input_sample_rate,
...
unsigned int AmAudio::resample(AmResamplingState &rstate, unsigned char *buffer, unsigned int s, int input_sample_rate,
                               int output_sample_rate)
{
...
unsigned int AmAudio::scaleSystemTS(unsigned long long system_ts)
{
    ...
    unsigned long long user_ts = system_ts * ((unsigned long long)getSampleRate() / 100) / (WALLCLOCK_RATE / 100);
...
{
    bool expected{ true };
    if (!has_pending_stereo_recorders.compare_exchange_strong(expected, false, std::memory_order_release,
