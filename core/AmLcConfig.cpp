...
static cfg_opt_t acl[]{ CFG_STR_LIST(PARAM_WHITELIST_NAME, 0, CFGF_NODEFAULT),
                        CFG_STR(PARAM_METHOD_NAME, "", CFGF_NODEFAULT), CFG_END() };

static cfg_opt_t rtsp[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT),
...
static cfg_opt_t dtls_client[] = {
    CFG_STR_LIST(PARAM_PROTOCOLS_NAME, 0, CFGF_NODEFAULT), CFG_STR_LIST(PARAM_PROFILES_NAME, 0, CFGF_NODEFAULT),
    CFG_STR(PARAM_CERTIFICATE_NAME, "", CFGF_NONE),        CFG_STR(PARAM_CERTIFICATE_KEY_NAME, "", CFGF_NONE),
    CFG_BOOL(PARAM_CERT_CHAIN_NAME, cfg_true, CFGF_NONE),  CFG_BOOL(PARAM_CERT_CN_NAME, cfg_true, CFGF_NONE),
    CFG_STR_LIST(PARAM_CA_LIST_NAME, 0, CFGF_NODEFAULT),   CFG_END()
};

static cfg_opt_t dtls_server[] = {
    CFG_STR_LIST(PARAM_PROTOCOLS_NAME, 0, CFGF_NODEFAULT), CFG_STR_LIST(PARAM_PROFILES_NAME, 0, CFGF_NODEFAULT),
    CFG_STR(PARAM_CERTIFICATE_NAME, "", CFGF_NODEFAULT),   CFG_STR(PARAM_CERTIFICATE_KEY_NAME, "", CFGF_NODEFAULT),
    CFG_BOOL(PARAM_VERIFY_CERT_NAME, cfg_true, CFGF_NONE), CFG_STR_LIST(PARAM_CIPHERS_NAME, 0, CFGF_NODEFAULT),
    CFG_STR_LIST(PARAM_MACS_NAME, 0, CFGF_NODEFAULT),      CFG_STR(PARAM_DH_PARAM_NAME, "", CFGF_NONE),
    CFG_STR_LIST(PARAM_CA_LIST_NAME, 0, CFGF_NODEFAULT),   CFG_END()
};

static cfg_opt_t dtls[] = { CFG_SEC(SECTION_CLIENT_NAME, dtls_client, CFGF_NODEFAULT),
                            CFG_SEC(SECTION_SERVER_NAME, dtls_server, CFGF_NODEFAULT), CFG_END() };

static cfg_opt_t sdes[] = { CFG_STR_LIST(PARAM_PROFILES_NAME, 0, CFGF_NODEFAULT), CFG_END() };

static cfg_opt_t zrtp[] = {
    CFG_STR_LIST(PARAM_HASHES_NAME, 0, CFGF_NODEFAULT),   CFG_STR_LIST(PARAM_CIPHERS_NAME, 0, CFGF_NODEFAULT),
    CFG_STR_LIST(PARAM_AUTHTAGS_NAME, 0, CFGF_NODEFAULT), CFG_STR_LIST(PARAM_DHMODES_NAME, 0, CFGF_NODEFAULT),
    CFG_STR_LIST(PARAM_SAS_NAME, 0, CFGF_NODEFAULT),      CFG_END()
};

static cfg_opt_t srtp[] = { CFG_BOOL(PARAM_ENABLE_SRTP_NAME, cfg_true, CFGF_NONE),
                            CFG_SEC(SECTION_SDES_NAME, sdes, CFGF_NONE),
                            CFG_SEC(SECTION_DTLS_NAME, dtls, CFGF_NODEFAULT),
                            CFG_SEC(SECTION_ZRTP_NAME, zrtp, CFGF_NODEFAULT), CFG_END() };

static cfg_opt_t rtp[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT),
...
static cfg_opt_t sip_tcp[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT),
...
static cfg_opt_t tls_client[] = { CFG_STR_LIST(PARAM_PROTOCOLS_NAME, 0, CFGF_NODEFAULT),
...
static cfg_opt_t tls_server[] = {
    CFG_STR_LIST(PARAM_PROTOCOLS_NAME, 0, CFGF_NODEFAULT),   CFG_STR(PARAM_CERTIFICATE_NAME, "", CFGF_NODEFAULT),
    CFG_STR(PARAM_CERTIFICATE_KEY_NAME, "", CFGF_NODEFAULT), CFG_BOOL(PARAM_VERIFY_CERT_NAME, cfg_true, CFGF_NONE),
    CFG_BOOL(PARAM_REQUIRE_CERT_NAME, cfg_true, CFGF_NONE),  CFG_STR_LIST(PARAM_CIPHERS_NAME, 0, CFGF_NODEFAULT),
    CFG_STR_LIST(PARAM_MACS_NAME, 0, CFGF_NODEFAULT),        CFG_STR(PARAM_DH_PARAM_NAME, "", CFGF_NONE),
    CFG_STR_LIST(PARAM_CA_LIST_NAME, 0, CFGF_NODEFAULT),     CFG_END()
};

static cfg_opt_t sip_tls[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT),
                               CFG_INT(PARAM_PORT_NAME, 0, CFGF_NODEFAULT),
                               CFG_BOOL(PARAM_USE_RAW_NAME, cfg_false, CFGF_NONE),
                               CFG_BOOL(PARAM_FORCE_OBD_IF_NAME, cfg_false, CFGF_NONE),
                               CFG_BOOL(PARAM_FORCE_VIA_PORT_NAME, cfg_false, CFGF_NONE),
...
static cfg_opt_t sip_wss[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT),
                               CFG_INT(PARAM_PORT_NAME, 0, CFGF_NODEFAULT),
                               CFG_BOOL(PARAM_USE_RAW_NAME, cfg_false, CFGF_NONE),
                               CFG_BOOL(PARAM_FORCE_OBD_IF_NAME, cfg_false, CFGF_NONE),
                               CFG_BOOL(PARAM_FORCE_VIA_PORT_NAME, cfg_false, CFGF_NONE),
...
static cfg_opt_t sip_ws[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT),
                              CFG_INT(PARAM_PORT_NAME, 0, CFGF_NODEFAULT),
                              CFG_BOOL(PARAM_USE_RAW_NAME, cfg_false, CFGF_NONE),
                              CFG_BOOL(PARAM_FORCE_OBD_IF_NAME, cfg_false, CFGF_NONE),
                              CFG_BOOL(PARAM_FORCE_TRANSPORT_NAME, cfg_true, CFGF_NONE),
...
static cfg_opt_t ip[] = {
    CFG_SEC(SECTION_RTSP_NAME, rtsp, CFGF_NODEFAULT),       CFG_SEC(SECTION_RTP_NAME, rtp, CFGF_NODEFAULT),
    CFG_SEC(SECTION_SIP_TCP_NAME, sip_tcp, CFGF_NODEFAULT), CFG_SEC(SECTION_SIP_UDP_NAME, sip_udp, CFGF_NODEFAULT),
    CFG_SEC(SECTION_SIP_TLS_NAME, sip_tls, CFGF_NODEFAULT), CFG_SEC(SECTION_SIP_WS_NAME, sip_ws, CFGF_NODEFAULT),
    CFG_SEC(SECTION_SIP_WSS_NAME, sip_wss, CFGF_NODEFAULT), CFG_END()
};

static cfg_opt_t interface[] = { CFG_FUNC("include", &cfg_include), CFG_STR(PARAM_DEFAULT_MEDIAIF_NAME, "", CFGF_NONE),
                                 CFG_SEC(SECTION_IP4_NAME, ip, CFGF_NODEFAULT),
                                 CFG_SEC(SECTION_IP6_NAME, ip, CFGF_NODEFAULT), CFG_END() };

static cfg_opt_t interfaces[] = { CFG_FUNC("include", &cfg_include),
                                  CFG_SEC(SECTION_IF_NAME, interface, CFGF_MULTI | CFGF_TITLE), CFG_END() };

static cfg_opt_t media_interface[] = { CFG_FUNC("include", &cfg_include), CFG_SEC(SECTION_IP4_NAME, ip, CFGF_NODEFAULT),
                                       CFG_SEC(SECTION_IP6_NAME, ip, CFGF_NODEFAULT),
                                       CFG_SEC(SECTION_SRTP_NAME, srtp, CFGF_NODEFAULT), CFG_END() };

static cfg_opt_t media_interfaces[] = { CFG_FUNC("include", &cfg_include),
                                        CFG_SEC(SECTION_IF_NAME, media_interface, CFGF_MULTI | CFGF_TITLE), CFG_END() };
...
/*                                            general section                                 */
/**********************************************************************************************/
static cfg_opt_t slimit[]{ CFG_INT(PARAM_LIMIT_NAME, VALUE_SESSION_LIMIT, CFGF_NONE),
                           CFG_INT(PARAM_CODE_NAME, VALUE_503_ERR_CODE, CFGF_NONE),
                           CFG_STR(PARAM_REASON_NAME, VALUE_SESSION_LIMIT_ERR, CFGF_NONE), CFG_END() };

static cfg_opt_t options_slimit[]{ CFG_INT(PARAM_LIMIT_NAME, VALUE_SESSION_LIMIT, CFGF_NONE),
                                   CFG_INT(PARAM_CODE_NAME, VALUE_503_ERR_CODE, CFGF_NONE),
                                   CFG_STR(PARAM_REASON_NAME, VALUE_SESSION_LIMIT_ERR, CFGF_NONE), CFG_END() };

static cfg_opt_t cps_limit[]{ CFG_INT(PARAM_LIMIT_NAME, 0, CFGF_NONE),
                              CFG_INT(PARAM_CODE_NAME, VALUE_503_ERR_CODE, CFGF_NONE),
                              CFG_STR(PARAM_REASON_NAME, VALUE_CPSLIMIT_ERR, CFGF_NONE), CFG_END() };

static cfg_opt_t sdm[]{ CFG_BOOL(PARAM_ALLOW_UAC_NAME, cfg_false, CFGF_NONE),
                        CFG_INT(PARAM_CODE_NAME, VALUE_503_ERR_CODE, CFGF_NONE),
                        CFG_STR(PARAM_REASON_NAME, VALUE_SDM_ERR_REASON, CFGF_NONE), CFG_END() };

static cfg_opt_t ice_cand_filter[]{ CFG_FUNC(FUNCTION_ALLOW_NAME, &ice_candidate_allow),
                                    CFG_FUNC(FUNCTION_DENY_NAME, &ice_candidate_deny), CFG_END() };

static cfg_opt_t general[] = { CFG_FUNC("include", &cfg_include),
...
/*                                         global section                                     */
/**********************************************************************************************/
static cfg_opt_t opt[] = { CFG_FUNC("include", &cfg_include),
...
static void cfg_error_callback(cfg_t *cfg, const char *fmt, va_list ap)
{
    ...
    if (cfg->title) {
        s +=
            snprintf(s, static_cast<size_t>(e - s), "%s:%d [%s/%s]: ", cfg->filename, cfg->line, cfg->name, cfg->title);
    } else {
        s += snprintf(s, static_cast<size_t>(e - s), "%s:%d [%s]: ", cfg->filename, cfg->line, cfg->name);
    }
...
int validate_dtmf_func(cfg_t *cfg, cfg_opt_t *opt)
{
    ...
    if (!valid) {
        ERROR("invalid value \'%s\' of the option \'%s\' - must be \'spandsp\' or \'internal\'", value.c_str(),
              opt->name);
    }
    return valid ? 0 : 1;
}
...
{
    std::string value = cfg_getstr(cfg, opt->name);
    bool valid = (value == VALUE_DISABLE || value == VALUE_OFF || value == VALUE_SUPPORTED || value == VALUE_REQUIRE);
    if (!valid) {
        ERROR("invalid value \'%s\' of the option \'%s\' - must be \'disabled\', \'supported\', \'require\' or \'off\'",
...
{
    std::string value = cfg_getstr(cfg, opt->name);
    bool        valid = (value == VALUE_LIBSAMPLERATE || value == VALUE_UNAVAILABLE || value == VALUE_INTERNAL);
    if (!valid) {
        ERROR("invalid value \'%s\' of the option \'%s\' - must be \'libsamplerate\', \'internal\' or \'unavailable\'",
              value.c_str(), opt->name);
    }
    return valid ? 0 : 1;
}
...
{
    std::string value = cfg_getstr(cfg, opt->name);
    bool        valid = (value == VALUE_PACKETS || value == VALUE_DELAY);
    if (!valid) {
...
int validate_tls_protocol_func(cfg_t *cfg, cfg_opt_t *opt)
{
    ...
    if (!valid) {
        ERROR("invalid value \'%s\' of the option \'%s\' - must be \'TLSv1.2\' or \'TLSv1.3\'", value.c_str(),
              opt->name);
    }
    return valid ? 0 : 1;
}
...
static int check_dir_write_permissions(const string &dir, const char *opt_name)
{
    ...
    if (!st.is_open()) {
        ERROR("failed to write test file in %s directory: %s", opt_name, dir.c_str());
        return 1;
    }
    st.close();
...
#define ENSURE_CFG_DIR_IS_WRITABLE(var)                                                                                \
    if (check_dir_write_permissions(config->var, #var))                                                                \
        return -1;
...
#define set_method_validator(IF_NAME, PROTO_NAME, PROTO_VERSION, METHOD_NAME)                                          \
    cfg_set_validate_func(                                                                                             \
        cfg, IF_NAME "|" SECTION_IF_NAME "|" PROTO_VERSION "|" PROTO_NAME "|" METHOD_NAME "|" PARAM_METHOD_NAME,       \
...
#define set_sip_acl_validator_proto(PROTO_NAME) set_acl_validator_proto(set_sip_acl_validator_proto_version, PROTO_NAME)
...
#define set_ip_func_validator(INTERFACE_NAME, PROTO_NAME)                                                              \
    cfg_set_validate_func(                                                                                             \
        cfg, INTERFACE_NAME "|" SECTION_IF_NAME "|" SECTION_IP6_NAME "|" PROTO_NAME "|" PARAM_ADDRESS_NAME,            \
        validate_ip6_func);                                                                                            \
    cfg_set_validate_func(                                                                                             \
        cfg, INTERFACE_NAME "|" SECTION_IF_NAME "|" SECTION_IP4_NAME "|" PROTO_NAME "|" PARAM_ADDRESS_NAME,            \
        validate_ip4_func);
...
#define set_tls_version_func_validator(INTERFACE_NAME, PROTO_NAME)                                                     \
    cfg_set_validate_func(cfg,                                                                                         \
                          INTERFACE_NAME "|" SECTION_IF_NAME "|" SECTION_IP6_NAME "|" PROTO_NAME                       \
                                         "|" SECTION_CLIENT_NAME "|" PARAM_PROTOCOLS_NAME,                             \
    ...
    cfg_set_validate_func(cfg,                                                                                         \
                          ...
                          validate_tls_protocol_func);                                                                 \
    cfg_set_validate_func(cfg,                                                                                         \
                          INTERFACE_NAME "|" SECTION_IF_NAME "|" SECTION_IP4_NAME "|" PROTO_NAME                       \
                                         "|" SECTION_CLIENT_NAME "|" PARAM_PROTOCOLS_NAME,                             \
                          validate_tls_protocol_func);                                                                 \
    cfg_set_validate_func(cfg,                                                                                         \
                          INTERFACE_NAME "|" SECTION_IF_NAME "|" SECTION_IP4_NAME "|" PROTO_NAME                       \
    ...
    cfg_set_validate_func(cfg,
                          SECTION_MEDIAIF_NAME "|" SECTION_IF_NAME "|" SECTION_IP6_NAME "|" SECTION_RTP_NAME
                                               "|" SECTION_SRTP_NAME "|" SECTION_DTLS_NAME "|" SECTION_SERVER_NAME
                                               "|" PARAM_PROTOCOLS_NAME,
                          validate_dtls_protocol_func);
    cfg_set_validate_func(cfg,
                          SECTION_MEDIAIF_NAME "|" SECTION_IF_NAME "|" SECTION_IP4_NAME "|" SECTION_RTP_NAME
                                               "|" SECTION_SRTP_NAME "|" SECTION_DTLS_NAME "|" SECTION_SERVER_NAME
                                               "|" PARAM_PROTOCOLS_NAME,
                          validate_dtls_protocol_func);
...
int AmLcConfig::readConfiguration(ConfigContainer *config)
{
    ...
    case CFG_FILE_ERROR:
        ERROR("failed to open configuration file: %s (%s)", config_path.c_str(), strerror(errno));
        return -1;
    case CFG_PARSE_ERROR: ERROR("failed to parse configuration file: %s", config_path.c_str()); return -1;
    default:              ERROR("got unexpected error on configuration file processing: %s", config_path.c_str()); return -1;
    }

    if (readGeneral(m_cfg, config) || readSigInterfaces(m_cfg, config) || readMediaInterfaces(m_cfg, config) ||
        readModules(m_cfg, config) || readRoutings(m_cfg, config) || checkSipInterfaces(config))
    {
        return -1;
    }
...
int AmLcConfig::readGeneral(cfg_t *cfg, ConfigContainer *config)
{
    ...
    if (config == &m_config) {
        ...
        trsp_socket::log_level_raw_msgs        = parse_log_level(cfg_getstr(gen, PARAM_LOG_RAW_NAME)).value_or(L_DBG);
        _SipCtrlInterface::log_parsed_messages = cfg_getbool(gen, PARAM_LOG_PARS_NAME);
        ...
        for (int t = STIMER_A; t < __STIMER_MAX; t++) {
            std::string timer_cfg = std::string("sip_timer_") + static_cast<char>(tolower(*timer_name(cuint(t))));
    ...
    if (value == VALUE_PACKETS)
        config->symmetric_rtp_mode = ConfigContainer::SM_RTP_PACKETS;
    else
        config->symmetric_rtp_mode = ConfigContainer::SM_RTP_DELAY;
    ...
    if (value == VALUE_SPANDSP)
        ...
        config->default_dtmf_detector = Dtmf::SEMSInternal;
    ...
    if (value == VALUE_DISABLE || value == VALUE_OFF)
        config->rel100 = Am100rel::REL100_DISABLED;
    else if (value == VALUE_SUPPORTED)
        config->rel100 = Am100rel::REL100_SUPPORTED;
    else if (value == VALUE_REQUIRE)
        config->rel100 = Am100rel::REL100_REQUIRE;
    ...
    if (value == VALUE_LIBSAMPLERATE)
        config->resampling_implementation_type = AmAudio::LIBSAMPLERATE;
    else if (value == VALUE_UNAVAILABLE)
        config->resampling_implementation_type = AmAudio::UNAVAILABLE;
    else if (value == VALUE_INTERNAL)
        config->resampling_implementation_type = AmAudio::INTERNAL_RESAMPLER;
...
int AmLcConfig::readModules(cfg_t *cfg, ConfigContainer *config)
{
    ...
    for_each_str_list(modules_, PARAM_PATH_NAME, [&config](auto path) { config->modules_path.emplace_back(path); });
                ...
                if (!p->public_domain.empty()) {
                    ...
                    if (resolver::instance()->resolve_name(p->public_domain, &dh, &a, priority) < 0) {
                        WARN("failed to resolve domain: '%s' with priority %s "
                             ...
                             p->transportToStr().data(), p->ipTypeToStr().data());
                    }
                }
            }
        }
            ...
            if (cfg_size(if_, SECTION_SRTP_NAME)) {
                ...
                if (!dtls) {
                    ...
                    for (unsigned int i = 0; i < cfg_size(server, PARAM_PROFILES_NAME); i++) {
                        media_if.srtp->server_settings.srtp_profiles.push_back(
                            SdpCrypto::str2profile(cfg_getnstr(server, PARAM_PROFILES_NAME, i)));
                    }
                    if (getMandatoryParameter(server, PARAM_CERTIFICATE_NAME,
                                              media_if.srtp->server_settings.certificate_path) ||
                        getMandatoryParameter(server, PARAM_CERTIFICATE_KEY_NAME,
                                              media_if.srtp->server_settings.certificate_key_path))
                    {
                        return -1;
                    }
                    ...
                    media_if.srtp->server_settings.verify_client_certificate =
                        cfg_getbool(server, PARAM_VERIFY_CERT_NAME);
                    ...
                    if (media_if.srtp->server_settings.verify_client_certificate &&
                        !media_if.srtp->server_settings.require_client_certificate)
                    {
                        ERROR("incorrect server tls configuration for interface %s: verify client certificate cannot "
                              "be set, if clients certificate is not required",
                              media_if.name.c_str());
                        return -1;
                    }
                    ...
                    for (unsigned int i = 0; i < cfg_size(client, PARAM_PROFILES_NAME); i++) {
                        media_if.srtp->client_settings.srtp_profiles.push_back(
                            SdpCrypto::str2profile(cfg_getnstr(client, PARAM_PROFILES_NAME, i)));
                    }
                    media_if.srtp->client_settings.certificate_path = cfg_getstr(client, PARAM_CERTIFICATE_NAME);
                    media_if.srtp->client_settings.certificate_key_path =
                        cfg_getstr(client, PARAM_CERTIFICATE_KEY_NAME);
                    media_if.srtp->client_settings.verify_certificate_chain =
                        cfg_getbool(client, PARAM_CERT_CHAIN_NAME);
                ...
                if (!self_p->public_domain.empty()) {
                    ...
                    if (resolver::instance()->resolve_name(self_p->public_domain, &dh, &a, priority) < 0) {
                        WARN("failed to resolve domain: '%s' with priority %s "
                             ...
                             self_p->transportToStr().data(), self_p->ipTypeToStr().data());
                    }
                }
                ...
                for (auto &i : config->media_ifs) {
                    ...
                    for (auto &p : i.proto_info) {
                        ...
                        if (p->local_ip != self_p->local_ip)
                            continue;
                        if (self_p->low_port <= p->high_port && p->low_port <= self_p->high_port) {
                            ERROR("media interface '%s' with range (%hu-%hu) for %s %s "
                                  "overlaps ports range for interface '%s' with range (%hu-%hu)",
                                  media_if.name.data(), self_p->low_port, self_p->high_port,
                                  self_p->ipTypeToStr().data(), self_p->local_ip.data(), i.name.data(), p->low_port,
                                  p->high_port);
                            return -1;
                        }
                    }
...
IP_info *AmLcConfig::readInterface(cfg_t *cfg, const std::string &if_name, AddressType ip_type)
{
    ...
    if (stlinfo) {
        ...
        {
            ERROR("incorrect server tls configuration for interface %s: verify client certificate cannot be set, if "
                  "clients certificate is not required",
                  if_name.c_str());
            return nullptr;
        }
...
int AmLcConfig::finalizeIpConfig(ConfigContainer *config)
{
    ...
    for (auto if_iterator = config->sip_ifs.begin(); if_iterator != config->sip_ifs.end(); if_iterator++) {
        ...
        if (if_names_iterator != config->sip_if_names.end()) {
            ...
            config->sip_if_names.insert(
                std::make_pair(if_iterator->name, static_cast<unsigned short>(if_iterator - config->sip_ifs.begin())));
        }
        ...
        for (auto &info : if_iterator->proto_info) {
            ...
            if (insertSIPInterfaceMapping(config, *info, cint(if_iterator - config->sip_ifs.begin())) < 0 ||
                (*if_iterator).insertProtoMapping(*info, i) || setNetInterface(config, *info))
            {
                return -1;
            }
            ...
            if (tls_info) {
                if (!tls_info->client_settings.checkCertificateAndKey(if_iterator->name.c_str(), "SIP", "client") ||
                    !tls_info->server_settings.checkCertificateAndKey(if_iterator->name.c_str(), "SIP", "server"))
                {
                    return -1;
                }
    ...
    for (auto if_iterator = config->media_ifs.begin(); if_iterator != config->media_ifs.end(); if_iterator++) {
        ...
        if (if_names_iterator != config->media_if_names.end()) {
            ...
            config->media_if_names.insert(std::make_pair(
                if_iterator->name, static_cast<unsigned short>(if_iterator - config->media_ifs.begin())));
        }

        if (if_iterator->srtp->dtls_enable) {
            if (!if_iterator->srtp->client_settings.checkCertificateAndKey(if_iterator->name.c_str(), "media",
                                                                           "client") ||
                !if_iterator->srtp->server_settings.checkCertificateAndKey(if_iterator->name.c_str(), "media",
                                                                           "server"))
            {
                return -1;
            }
        ...
        for (auto &info : if_iterator->proto_info) {
            ...
            if ((*if_iterator).insertProtoMapping(*info, i) || setNetInterface(config, *info)) {
                return -1;
            }
        ...
        for (; info_it != config->sip_ifs[idx].proto_info.end(); info_it++) {
            ...
            for (; intf_it != config->sys_ifs.end(); ++intf_it) {
                ...
                if (config->local_sip_ip2if.find(intf_it->addrs.front().addr) == config->local_sip_ip2if.end()) {
                    DBG("mapping unmapped IP address '%s' to interface #%u ", intf_it->addrs.front().addr.c_str(), idx);
                    config->local_sip_ip2if[intf_it->addrs.front().addr] = static_cast<unsigned short>(idx);
                }
            }
        }
    }
...
int AmLcConfig::setNetInterface(ConfigContainer *config, IP_info &ip_if)
{
    ...
    ERROR("failed to find interface with address: %s", ip_if.local_ip.c_str());
    return -1;
}
...
std::string AmLcConfig::fixIface2IP(const std::string &dev_name, bool v6_for_sip, ConfigContainer *config)
{
    ...
    for (std::vector<SysIntf>::iterator intf_it = config->sys_ifs.begin(); intf_it != config->sys_ifs.end(); ++intf_it)
    {
...
bool AmLcConfig::fillSysIntfList(ConfigContainer *config)
{
    ...
    for (struct ifaddrs *p_if = ifap; p_if != nullptr; p_if = p_if->ifa_next) {
        ...
        intf_it->addrs.push_back(IPAddr(fixIface2IP(host, true), static_cast<short>(p_if->ifa_addr->sa_family)));
    }
        ...
        for (auto &info : sip_if.proto_info) {
            for (auto &other_info_tuple : infos) {
                ...
                if (info->local_ip == other_info.local_ip && info->local_port == other_info.local_port &&
                    (info->type == other_info.type || (info->type > SIP_info::UDP && other_info.type > SIP_info::UDP)))
                {
                    ERROR("duplicate ip/port %s/%d in interfaces %s/%s and %s/%s", other_info.local_ip.c_str(),
                          other_info.local_port, sip_if.name.c_str(), info->transportToStr().c_str(),
