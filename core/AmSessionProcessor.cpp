...
vector<AmSessionProcessorThread *>::iterator AmSessionProcessor::threads_it = AmSessionProcessor::threads.begin();

AmSessionProcessorIterateRequestContext::AmSessionProcessorIterateRequestContext(iterate_func_cb_type icb,
                                                                                 finish_func_cb_type  fcb,
                                                                                 void *user_data, int threads_count)
...
void AmSessionProcessor::sendIterateRequest(AmSessionProcessorIterateRequestContext::iterate_func_cb_type icb,
                                            AmSessionProcessorIterateRequestContext::finish_func_cb_type  fcb,
                                            void                                                         *callback_ptr)
{
    threads_mut.lock();

    auto ctx = new AmSessionProcessorIterateRequestContext(icb, fcb, callback_ptr, threads.size());

    int i = 0;
    for (auto &t : threads)
        t->sendIterateRequest(new AmSessionProcessorIterateRequestEvent(ctx, i++));
...
void AmSessionProcessorThread::run()
{
    ...
    while (true) {
        ...
        if (!startup_sessions.empty()) {
            ...
            for (std::vector<AmSession *>::iterator it = startup_sessions.begin(); it != startup_sessions.end(); it++) {
                DBG3("starting up [%s|%s]: [%p]", (*it)->getCallID().c_str(), (*it)->getLocalTag().c_str(), *it);
        ...
        if (fin_sessions.size()) {
            ...
            for (std::vector<AmSession *>::iterator it = fin_sessions.begin(); it != fin_sessions.end(); it++) {
                DBG3("finalizing session [%p/%s/%s]", *it, (*it)->getCallID().c_str(), (*it)->getLocalTag().c_str());
...
{
    DBG3("requesting session to stop.");
...
{
    AmSessionProcessorThreadAddEvent *add_ev = dynamic_cast<AmSessionProcessorThreadAddEvent *>(e);
    ...
    if (auto req_ev = dynamic_cast<AmSessionProcessorIterateRequestEvent *>(e)) {
        auto ctx = req_ev->ctx;

        // apply callback for all sessions
        for (auto &session : sessions) {
            ctx->iterate_callback(session, ctx->user_data, req_ev->ret);
        }

        if (ctx->threads_awaited.dec_and_test()) {
            // this thread is the last that finished the iteration
            ctx->finish_callback(ctx->aggregated_ret, ctx->user_data);
