...
class SingleSubscription {
  public:
    enum Role { Subscriber = 0, Notifier };
  ...
  public:
    ...
    SingleSubscription(AmSipSubscription *subs, Role role, const string &event, const string &id);
    ...
    void   sendReferNotify(AmBasicSipDialog *sip_dlg, string &body, bool terminate_sub = false,
...
class AmSipSubscription {
  protected:
    ...
    Subscriptions::iterator findSubscription(SingleSubscription::Role role, const string &event, const string &id);
    ...
    virtual SingleSubscription *newSingleSubscription(SingleSubscription::Role role, const string &event,
                                                      const string &id);
  ...
  public:
     ...
     */
    bool subscriptionExists(SingleSubscription::Role role, const string &event, const string &id);
    ...
    virtual void onNotify([[maybe_unused]] const AmSipRequest &req, [[maybe_unused]] SingleSubscription *sub) {}

    virtual void onFailureReply([[maybe_unused]] const AmSipReply &reply, [[maybe_unused]] SingleSubscription *sub) {}

    virtual void onTimeout(int timer_id, SingleSubscription *sub);

    virtual void debug();

    bool sendReferNotify(AmBasicSipDialog *sip_dlg, const string &id, string &body, bool terminate_sub = false,
                         const string &reason = string());
};
...
struct SIPSubscriptionEvent : public AmEvent {
    ...
    SIPSubscriptionEvent(SubscriptionStatus status, const string &handle, unsigned int expires = 0,
...
struct AmSipSubscriptionInfo {
    ...
    AmSipSubscriptionInfo(const string &domain, const string &user, const string &from_user, const string &pwd,
...
class AmSipSubscriptionDialog : public AmBasicSipDialog, public AmBasicSipEventHandler, public AmSipSubscription {
    string event;
    string event_id;
    string accept;
    string sess_link;
  ...
  public:
    AmSipSubscriptionDialog(const AmSipSubscriptionInfo &info, const string &sess_link, AmEventQueue *ev_q = NULL);
    ...
    void onSipRequest(const AmSipRequest &req) { AmSipSubscription::onRequestIn(req); }

    void onRequestSent(const AmSipRequest &req) { AmSipSubscription::onRequestSent(req); }
    ...
    void onReplySent(const AmSipRequest &req, const AmSipReply &reply) { AmSipSubscription::onReplySent(req, reply); }
};
