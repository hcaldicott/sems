...
class DTLSUDPTLConnection : public AmStreamConnection {
  ...
  public:
    DTLSUDPTLConnection(AmMediaTransport *_transport, const string &remote_addr, int remote_port,
...
struct FaxCompleteEvent : public AmEvent {
    FaxCompleteEvent(bool isSuccess, const string &strResult, const map<string, string> &stat,
                     const map<string, string> &t38params)
        : AmEvent(0)
        , m_isSuccess(isSuccess)
        , m_strResult(strResult)
        , m_statistics(stat)
        , m_t38Params(t38params)
    {
    }
    bool                m_isSuccess;
    string              m_strResult;
