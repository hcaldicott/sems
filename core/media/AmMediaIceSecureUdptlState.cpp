...
AmMediaIceSecureUdptlState::AmMediaIceSecureUdptlState(AmMediaTransport *transport)
    ...
    , AmMediaSecureUdptlState(transport)
{
}
...
void AmMediaIceSecureUdptlState::addConnections(const AmMediaStateArgs &args)
{
    ...
    auto                pred      = [&](auto conn, bool &stop) {
        if (conn->getConnType() == AmStreamConnection::UDPTL_CONN && conn->getRHost() == *args.address &&
            conn->getRPort() == *args.port)
        {
            stop   = true;
            exists = true;
        } else if (conn->getConnType() == AmStreamConnection::DTLS_CONN && conn->getRHost() == *args.address &&
                   conn->getRPort() == *args.port)
        {
            dtls_conn = conn;
        }
    };
    transport->iterateConnections({ AmStreamConnection::UDPTL_CONN, AmStreamConnection::DTLS_CONN }, pred);
    if (exists)
        return;
    ...
    CLASS_DBG("add dtls-udptl connection, state:%s, type:%s, raddr:%s, rport:%d", state2str(), transport->type2str(),
              args.address.value().c_str(), *args.port);
    new_conns.push_back(transport->getConnFactory()->createDtlsUdptlConnection(*args.address, *args.port, dtls_conn));
