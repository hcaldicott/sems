...
{
    CLASS_DBG("add pair in ice context %s:%u/%s:%u, priority %u", conn->getTransport()->getLocalIP().c_str(),
              conn->getTransport()->getLocalPort(), conn->getRHost().c_str(), conn->getRPort(), conn->getPriority());
...
{
    CLASS_DBG("update pair in ice context %s:%u/%s:%u, priority %u", conn->getTransport()->getLocalIP().c_str(),
              conn->getTransport()->getLocalPort(), conn->getRHost().c_str(), conn->getRPort(), conn->getPriority());
...
sockaddr_storage *IceContext::getAllowedIceAddr(int family)
{
    ...
    for (auto &pair : pairs) {
        AmStunConnection::PairState state = pair.second->getState();
        sockaddr_storage           *sa    = &current_family_addr[family];
        if (state == AmStunConnection::PAIR_SUCCEEDED && family == pair.second->getTransport()->getLocalAddrFamily()) {
...
AmStunConnection::AmStunConnection(AmMediaTransport *_transport, const string &remote_addr, int remote_port,
                                   unsigned int _lpriority, unsigned int _priority)
...
void AmStunConnection::set_credentials(const string &luser, const string &lpassword, const string &ruser,
...
{
    switch (state) {
    case PAIR_FROZEN:      return "FROZEN";
    case PAIR_WAITING:     return "WAITING";
    case PAIR_IN_PROGRESS: return "IN_PROGRESS";
    case PAIR_RETRANSMIT:  return "RETRANSMIT";
    case PAIR_FAILED:      return "FAILED";
    case PAIR_SUCCEEDED:   return "SUCCEEDED";
    }
}
...
void AmStunConnection::send_request(StunTransactionId trans_id)
{
    ...
    if (lpriority >= (1U << 31) || lpriority == 0) {
        WARN("stun priority (0x%x) is incorrect. raddr: %s:%hu", lpriority, am_inet_ntop(&r_addr).data(),
             am_get_port(&r_addr));
    }
...
void AmStunConnection::checkState()
{
    ...
    if (state == PAIR_FROZEN) {
        state = PAIR_WAITING;
        return;
    } else if (state == PAIR_SUCCEEDED || state == PAIR_FAILED || state == PAIR_WAITING) {
        count = 0;
