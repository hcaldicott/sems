...
std::shared_ptr<Botan::Private_Key> dtls_conf::private_key_for(const Botan::X509_Certificate &, const string &,
                                                               const string &)
{
    if (key) {
        return key;
...
vector<Botan::X509_Certificate> dtls_conf::cert_chain(const vector<string> &cert_key_types,
                                                      const std::vector<Botan::AlgorithmIdentifier> &, const string &,
                                                      const string &)
{
...
{
    expires = DTLS_TIMER_INTERVAL_MS / (TIMER_RESOLUTION / DTLS_TIMER_INTERVAL_MS) + wheeltimer::instance()->wall_clock;
...
void RtpSecureContext::initContext(const string &host, int port, shared_ptr<dtls_conf> settings,
                                   bool reinit) noexcept(false)
{
    ...
    try {
        ...
        if (dtls_settings->is_client()) {
            dtls_channel = new Botan::TLS::Client(
                tls_callbacks_proxy, session_manager_dtls::instance()->ssm, dtls_settings, dtls_settings, rand_gen,
                Botan::TLS::Server_Information(host.c_str(), port), Botan::TLS::Protocol_Version::DTLS_V12);
        } else {
            dtls_channel = new Botan::TLS::Server(tls_callbacks_proxy, session_manager_dtls::instance()->ssm,
                                                  dtls_settings, dtls_settings, rand_gen, true);
        }
...
void RtpSecureContext::tls_verify_cert_chain(const std::vector<Botan::X509_Certificate>              &cert_chain,
                                             ...
                                             const Botan::TLS::Policy &policy)
{
    ...
    std::transform(fingerprint.hash.begin(), fingerprint.hash.end(), fingerprint.hash.begin(),
...
AmDtlsConnection::AmDtlsConnection(AmMediaTransport *_transport, const string &remote_addr, int remote_port,
                                   DtlsContext *context)
...
void AmDtlsConnection::handleConnection(uint8_t *data, unsigned int size, struct sockaddr_storage *, struct timeval)
{
    try {
...
{
    assert(transport);
    return transport->send(&r_addr, (unsigned char *)data, size, AmStreamConnection::DTLS_CONN);
}
