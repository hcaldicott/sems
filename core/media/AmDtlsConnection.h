...
class dtls_conf : public Botan::TLS::Datagram_Policy, public Botan::Credentials_Manager {
  ...
  public:
    ...
    vector<Botan::X509_Certificate> cert_chain(const vector<string>                          &cert_key_types,
                                               const std::vector<Botan::AlgorithmIdentifier> &cert_signature_schemes,
                                               const string &type, const string &context) override;
...
struct DtlsContext {
    ...
    virtual void              initContext(const string &host, int port, shared_ptr<dtls_conf> settings,
                                          bool reinit = false) noexcept(false)                                     = 0;
    ...
    virtual bool              getDtlsKeysMaterial(srtp_profile_t &srtp_profile, vector<uint8_t> &local_key,
...
class RtpSecureContext : public Botan::TLS::Callbacks, public DtlsContext {
  ...
  public:
    ...
    void initContext(const string &host, int port, shared_ptr<dtls_conf> settings,
                     bool reinit = false) noexcept(false) override;
    ...
    bool getDtlsKeysMaterial(srtp_profile_t &srtp_profile, vector<uint8_t> &local_key,
    ...
    void tls_ssl_key_log_data(std::string_view label, std::span<const uint8_t> client_random,
                              std::span<const uint8_t> secret) const override;
};

class AmDtlsConnection : public AmStreamConnection {
  ...
  public:
    AmDtlsConnection(AmMediaTransport *transport, const string &remote_addr, int remote_port, DtlsContext *context);
