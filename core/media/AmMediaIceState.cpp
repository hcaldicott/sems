...
void AmMediaIceState::addStunConnections(const vector<SdpIceCandidate> *candidates, bool sdp_offer_owner)
{
    ...
    for (auto candidate : *candidates) {
        ...
        auto pred = [&](auto conn) {
            return conn->getConnType() == AmStreamConnection::STUN_CONN && conn->getRHost() == address &&
                   conn->getRPort() == port;
        };
        ...
        try {
            ...
            CLASS_DBG("add stun connection, state:%s, type:%s, raddr:%s, rport:%d", state2str(), transport->type2str(),
                      address.c_str(), port);
            auto conn = (AmStunConnection *)transport->getConnFactory()->createStunConnection(
                address, port, transport->getConnFactory()->ice_cred.lpriority, candidate.priority);
...
void AmMediaIceState::removeStunConnections()
{
    CLASS_DBG("removeConnections, conn_type:%s, state:%s, type:%s",
              AmStreamConnection::connType2Str(AmStreamConnection::STUN_CONN).c_str(), state2str(),
              transport->type2str());
...
AmMediaState *AmMediaIceState::allowStunConnection(const sockaddr_storage *remote_addr, uint32_t priority)
{
    ...
    CLASS_DBG("allow stun connection by addr: %s, port: %d, state: %s, type: %s", address.c_str(), port, state2str(),
              transport->type2str());
...
AmMediaState *AmMediaIceState::allowStunPair(const sockaddr_storage *remote_addr)
{
    const string address = am_inet_ntop(remote_addr);
    const int    port    = am_get_port(remote_addr);
    CLASS_DBG("allow stun pair by addr: %s, port: %d, state: %s, type: %s", address.c_str(), port, state2str(),
              transport->type2str());
...
void AmMediaIceState::resetCurRtpConnection()
{
    ...
    transport->findConnection(
        [&](auto conn) {
            return conn->getConnType() == AmStreamConnection::ZRTP_CONN && conn->isAddrConnection(target_addr);
        },
