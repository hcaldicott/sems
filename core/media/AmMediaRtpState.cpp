...
void AmMediaRtpState::addConnections(const AmMediaStateArgs &args)
{
    if (!args.address || !args.port || !args.family)
        return;
    if (args.family.value() != transport->getLocalAddrFamily())
        return;

    // check is rtp connection already exists
    auto pred = [&](auto conn) {
        return conn->getConnType() == AmStreamConnection::RTP_CONN && conn->getRHost() == *args.address &&
    ...
    if (transport->getTransportType() == RTP_TRANSPORT) {
        CLASS_DBG("add rtp connection, state:%s, type:%s, raddr:%s, rport:%d", state2str(), transport->type2str(),
                  args.address.value().c_str(), *args.port);
        transport->addConnection(transport->getConnFactory()->createRtpConnection(*args.address, *args.port));
    }

    CLASS_DBG("add rtcp connection, state:%s, type:%s, raddr:%s, rport:%d", state2str(), transport->type2str(),
              args.address.value().c_str(), *args.port);
    transport->addConnection(transport->getConnFactory()->createRtcpConnection(*args.address, *args.port));
}
...
void AmMediaRtpState::updateConnections(const AmMediaStateArgs &args)
{
    ...
    if (transport->getCurRtpConn()) {
        ...
        transport->iterateConnections(AmStreamConnection::RTP_CONN,
    ...
    if (transport->getCurRtcpConn()) {
        ...
        transport->iterateConnections(AmStreamConnection::RTCP_CONN,
                                      [&](auto conn, bool &stop) { conn->setRAddr(*args.address, *args.port); });
    }
