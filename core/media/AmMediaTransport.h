...
class AmMediaTransport : public AmMediaConnectionsHolder,
                         public AmRtpSession
...
#endif
{
  ...
  public:
    ...
    int getComponentId()
    {
        return type == RTCP_TRANSPORT ? 2 : 1;
    }
    ...
    AddressType getLocalAddrType()
    {
        return getLocalAddrFamily() == AF_INET ? AT_V4 : AT_V6;
    }
    int getLocalAddrFamily()
    {
        return l_saddr.ss_family;
    }
    ...
    virtual ssize_t send(sockaddr_storage * raddr, unsigned char *buf, int size,
  ...
  public:
    int getSrtpCredentialsBySdp(const SdpMedia &local_media, const SdpMedia &remote_media, string &local_key,
