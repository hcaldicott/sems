...
{
    int cprofile =
        transport->getSrtpCredentialsBySdp(local_media, remote_media, srtp_cred.local_key, srtp_cred.remote_keys);
    if (cprofile < 0)
        return -1;
...
{
    return createSrtpConnection(raddr, rport, srtp_cred.srtp_profile, srtp_cred.local_key, srtp_cred.remote_keys,
                                false);
}

AmStreamConnection *AmMediaConnectionFactory::createSrtpConnection(const string &raddr, int rport, int srtp_profile,
                                                                   const string           &local_key,
                                                                   const srtp_master_keys &remote_keys, bool rtcp)
{
    try {
        AmSrtpConnection *conn = new AmSrtpConnection(
            transport, raddr, rport, rtcp ? AmStreamConnection::RTCP_CONN : AmStreamConnection::RTP_CONN);
...
{
    return createSrtpConnection(raddr, rport, srtp_cred.srtp_profile, srtp_cred.local_key, srtp_cred.remote_keys, true);
}
...
AmStreamConnection *AmMediaConnectionFactory::createDtlsUdptlConnection(const string &raddr, int rport,
