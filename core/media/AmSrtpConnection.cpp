...
srtp_master_keys::strp_mater_key_container::strp_mater_key_container(const string &key_, uint32_t mki_id_,
                                                                     uint32_t mki_size_)
    : key_data(key_)
    , mki_id_data(mki_id_)
{
...
srtp_master_keys::srtp_master_keys(const string &key, uint32_t mki_id, uint32_t mki_size)
{
...
void srtp_master_keys::add(const string &key, uint32_t mki_id, uint32_t mki_size)
{
    auto &d = data.emplace_back(key, mki_id, mki_size);
    srtp_data_ptrs.emplace_back(&d);
}

AmSrtpConnection::AmSrtpConnection(AmMediaTransport *_transport, const string &remote_addr, int remote_port,
                                   AmStreamConnection::ConnectionType conn_type)
...
void AmSrtpConnection::use_keys(srtp_profile_t profile, const string &tx_key, const srtp_master_keys &rx_keys)
{
...
void AmSrtpConnection::update_keys(srtp_profile_t profile, const string &tx_key, const srtp_master_keys &rx_keys)
{
    if (is_valid_keys(profile, tx_key, rx_keys) < 0)
        return;
    ...
    do { // scope for session_rx_mutex
        ...
        if (auto ret = srtp_update_stream(srtp_rx_session, &rx_policy); ret != srtp_err_status_ok) {
            ERROR("srtp_update_stream error %d", ret);
        }
    } while (0);
}

int AmSrtpConnection::is_valid_keys(srtp_profile_t profile, const string &tx_key, const srtp_master_keys &rx_keys)
{
...
std::string AmSrtpConnection::gen_base64(unsigned int key_s_len)
{
    ...
    while (len != key_s_len) {
        ...
        if (key_s_len < len + random_uuid.binary_value().size()) {
            data.insert(data.end(), random_uuid.binary_value().begin(),
                        random_uuid.binary_value().begin() + (key_s_len - len));
        } else {
...
int AmSrtpConnection::ensure_rx_stream_context(uint32_t ssrc_net_order)
{
    if (!rx_context_initialized) {
        ...
        CLASS_DBG("remove old SRTP context for SSRC: 0x%x", ntohl(last_rx_ssrc_net_order));
    ...
    CLASS_DBG("add %s receive stream context. SSRC:0x%x", getConnType() == RTP_CONN ? "RTP" : "RTCP",
...
void AmSrtpConnection::handleConnection(uint8_t *data, unsigned int size, struct sockaddr_storage *recv_addr,
...

    {
        ...
        if (last_rx_ssrc_net_order != rx_ssrc_net_order) {
            ...
            if (last_rx_ssrc_net_order) {
                CLASS_DBG("SSRC changed 0x%x -> 0x%x. add new SRTP stream context", ntohl(last_rx_ssrc_net_order),
